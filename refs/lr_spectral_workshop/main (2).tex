...
Integrate by parts cell-by-cell in the direction $\theta$.  Interior affine terms cancel across adjacent cells because $f$ is continuous, while derivative jumps remain on cell boundaries.  Repeating the boundary reduction localizes the $q$th contribution on codimension-$q$ faces.  A codimension-$q$ face is reached after $q$ boundary reductions plus one affine derivative, producing the factor $\rho^{-(q+1)}$.  This recovers the anisotropic structure identified by Rahaman et al.~\cite{rahaman2019spectral}: in almost all directions ReLU networks have fast $k^{-(d+1)}$ amplitude decay, but in directions orthogonal to facets the decay can be as slow as $k^{-2}$.  The slow high-frequency directions are therefore tied to low-codimension faces, while high-codimension intersections mostly add faster-decaying shell terms.
\end{proof}
...
\subsection{\texorpdfstring{E3: width $2^{15}$ rank sweep in Fourier space}{E3: width 2^15 rank sweep in Fourier space}}
...
The main empirical signature of Proposition~4.2 is not that rank should keep helping forever.  The shell law predicts a coefficient-level saturation: once the high-codimension moments relevant to the observed bandwidth have been suppressed, adding more rank should not further flatten the useful log--log Fourier scaling.  We therefore compute a \emph{useful scaling exponent} from the recovery slope and mark the first rank that reaches the plateau while also minimizing the finite-budget risk.  In both sparse and dense mixtures this plateau rank is $r=4$.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/plateau.png}
\caption{Plateau diagnostic for the Fourier scaling law.  Solid curves show the useful log--log scaling exponent extracted from recovery; dotted curves show the raw fitted exponent.  The dashed vertical line marks $r=4$, where the useful coefficient has saturated and the marginal spectral gain drops to zero.  This is the operational sweet spot: ranks below $4$ are still approximation-limited, while ranks above $4$ add complexity without improving the finite-bandwidth scaling law.}
\label{fig:plateau}
\end{figure*}
...
\item The optimal rank is intermediate.  Ranks $1$--$2$ are too restrictive; full rank is too spectrally biased at finite training time; rank $4$ is the first plateau point where the useful log--log scaling coefficient stops improving while the approximation error is already controlled.
\end{itemize}
