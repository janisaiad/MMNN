...
\tikzstyle{box}=[draw,rounded corners,align=center,minimum height=0.80cm,text width=0.19\textwidth,fill=blue!4]
\tikzstyle{smallbox}=[draw,rounded corners,align=center,minimum height=0.70cm,text width=0.16\textwidth,fill=green!4]
\node[box] (rank) {Low-rank factors\\$W_\ell=U_\ell V_\ell^\top$};
\node[box,right=of rank] (path) {Path-space constraint\\mode-rank $\le r_\ell$};
\node[box,right=of path] (normals) {Normal subspaces\\$n_{\ell i}\in \cV_{\ell}$};
\node[box,right=of normals] (faces) {Fewer high-codim\\CPWL intersections};
\node[box,right=of faces] (fourier) {Flatter finite-scale\\Fourier shell law};
\draw[->] (rank) -- (path);
\draw[->] (path) -- (normals);
\draw[->] (normals) -- (faces);
\draw[->] (faces) -- (fourier);
\node[smallbox,below=0.82cm of faces] (opt) {But too low rank\\hurts approximation};
\draw[->,dashed] (opt) -- (faces);
\node[smallbox,below=0.82cm of fourier] (sweet) {Sweet spot: best\\approx./optimization trade-off};
\draw[->,dashed] (opt) -- (sweet);
\draw[->,dashed] (fourier) -- (sweet);
\end{tikzpicture}}
...
\begin{proposition}[path tensor rank]
Let $\mathcal{T}_\varepsilon$ be the active path coefficient tensor of the affine gradient $a_\varepsilon$.  For every hidden layer $\ell$, the mode-$\ell$ matricization of $\mathcal{T}_\varepsilon$ has rank at most $r_\ell$.  Consequently the dimension of the active path-coefficient span is bounded by a product of ranks rather than a product of widths.
\end{proposition}

\paragraph{Interpretation.}
The number of symbolic paths can remain large, but the coefficients attached to these paths live in a low-dimensional algebraic variety.  This is the correct meaning of ``low rank creates fewer paths'' for a functional analysis of the network.
...
N_q(m,\rho)\le {m\choose q}\sum_{j=0}^{\rho-q}{m-q\choose j},\qquad q\le \rho,
\end{equation}
...
M_q(r)\lesssim C_q(r) {m\choose q}\sum_{j=0}^{\rho(r)-q}{m-q\choose j},
\end{equation}
...
\end{equation}
If $M_2,M_3,\ldots$ are suppressed relative to $M_1$, then $\alpha_{\rm eff}$ moves toward the facet exponent $4$.  This is the precise version of ``low rank produces lower spectral decay.''
...
\begin{groupplot}[
  ...
  width=0.31\textwidth,height=0.23\textwidth,
  grid=both,grid style={gray!15},
  xmin=1,xmax=50]
\nextgroupplot[xlabel={rank $r$},ylabel={$M_2/M_1$},ymode=log]
\addplot+[mark=none,thick] table[col sep=comma,x=rank,y=M2_over_M1]{figdata/theory_face_moments.csv};
\nextgroupplot[xlabel={rank $r$},ylabel={$M_3/M_1$},ymode=log]
\addplot+[mark=none,thick] table[col sep=comma,x=rank,y=M3_over_M1]{figdata/theory_face_moments.csv};
\nextgroupplot[xlabel={rank $r$},ylabel={predicted slope at $\rho=8$}]
\addplot+[mark=none,thick] table[col sep=comma,x=rank,y=slope_shell8]{figdata/theory_face_moments.csv};
\end{groupplot}
\end{tikzpicture}
...
\wh f(k)= -\frac{1}{(ik)^2}\sum_j \Delta_j e^{-ik t_j}+O(|k|^{-3}),
\quad
...
\section{Experiments}
All plots are native \LaTeX/PGFPlots generated from CSV files.  The code releases the deterministic rank sweep used for the figures.  The width-$2^{15}$ experiment is a Fourier/kernel-limit surrogate for a two-hidden-layer CPWL random-feature model: ranks $1,\ldots,50$ control the path-space bottleneck, and the full-rank endpoint is the dense/full transfer limit.  This avoids materializing a dense $32768\times32768$ hidden matrix while preserving the Fourier transfer calculation.
...
\begin{groupplot}[
 ...
 width=0.235\textwidth,height=0.23\textwidth,
 grid=both,grid style={gray!15},
 xtick=data, xticklabel style={rotate=45,anchor=east},
 symbolic x coords={full,32,16,8,4,3,2,1}]
\nextgroupplot[xlabel={rank},ylabel={8-way junction proxy}]
\addplot+[mark=*] table[col sep=comma,x=rank_label,y=junction8]{figdata/cpwl3d_summary.csv};
\nextgroupplot[xlabel={rank},ylabel={4-way junction proxy}]
\addplot+[mark=*] table[col sep=comma,x=rank_label,y=junction4]{figdata/cpwl3d_summary.csv};
\nextgroupplot[xlabel={rank},ylabel={unique regions}]
\addplot+[mark=*] table[col sep=comma,x=rank_label,y=unique_regions]{figdata/cpwl3d_summary.csv};
\nextgroupplot[xlabel={rank},ylabel={shell slope}]
\addplot+[mark=*] table[col sep=comma,x=rank_label,y=shell_slope]{figdata/cpwl3d_summary.csv};
\end{groupplot}
\end{tikzpicture}
...
\subsection{E2: 1D raw tails stay near $-4$}
...
\centering
\begin{tikzpicture}
\begin{axis}[width=0.98\columnwidth,height=0.58\columnwidth,
 grid=both,grid style={gray!15},
 symbolic x coords={full,32,16,8,4,3,2,1},xtick=data,xticklabel style={rotate=45,anchor=east},
 xlabel={rank},ylabel={1D raw tail exponent}]
\addplot+[mark=*] table[col sep=comma,x=rank_label,y=exponent]{figdata/cpwl1d_tail_sanity.csv};
\addplot[dashed,domain=full:1] coordinates {(full,-4) (1,-4)};
\end{axis}
\end{tikzpicture}
...
\subsection{E3: width $2^{15}$ rank sweep in Fourier space}
...
\begin{groupplot}[
 ...
 width=0.43\textwidth,height=0.27\textwidth,
 grid=both,grid style={gray!15},xmin=1,xmax=50]
\nextgroupplot[ylabel={sparse test MSE},xlabel={rank}]
\addplot+[mark=none,thick] table[col sep=comma,x=rank,y=test_mse]{figdata/wide32768_sparse_rank_sweep.csv};
\addplot[dashed,thick] table[col sep=comma,x=rank,y=test_mse]{figdata/wide32768_sparse_full_lines.csv};
\nextgroupplot[ylabel={sparse recovery slope},xlabel={rank}]
\addplot+[mark=none,thick] table[col sep=comma,x=rank,y=recovery_slope]{figdata/wide32768_sparse_rank_sweep.csv};
\addplot[dashed,thick] table[col sep=comma,x=rank,y=recovery_slope]{figdata/wide32768_sparse_full_lines.csv};
\nextgroupplot[ylabel={dense test MSE},xlabel={rank}]
\addplot+[mark=none,thick] table[col sep=comma,x=rank,y=test_mse]{figdata/wide32768_dense_rank_sweep.csv};
\addplot[dashed,thick] table[col sep=comma,x=rank,y=test_mse]{figdata/wide32768_dense_full_lines.csv};
\nextgroupplot[ylabel={dense recovery slope},xlabel={rank}]
\addplot+[mark=none,thick] table[col sep=comma,x=rank,y=recovery_slope]{figdata/wide32768_dense_rank_sweep.csv};
\addplot[dashed,thick] table[col sep=comma,x=rank,y=recovery_slope]{figdata/wide32768_dense_full_lines.csv};
\end{groupplot}
\end{tikzpicture}
...
\subsection{E4: mode-wise recovery and training curves}
...
\begin{tikzpicture}
\begin{groupplot}[
 group style={group size=2 by 2,horizontal sep=1.1cm,vertical sep=1.0cm},
 width=0.43\textwidth,height=0.25\textwidth,
 grid=both,grid style={gray!15}]
\nextgroupplot[xlabel={frequency},ylabel={sparse recovery},xmode=log,ymode=log,legend pos=south west]
\addplot+[mark=*] table[col sep=comma,x=frequency,y=recovery]{figdata/wide32768_recovery_sparse_4.csv};\addlegendentry{$r=4$}
\addplot+[mark=square*] table[col sep=comma,x=frequency,y=recovery]{figdata/wide32768_recovery_sparse_8.csv};\addlegendentry{$r=8$}
\addplot+[mark=triangle*] table[col sep=comma,x=frequency,y=recovery]{figdata/wide32768_recovery_sparse_full.csv};\addlegendentry{full}
\nextgroupplot[xlabel={frequency},ylabel={dense recovery},xmode=log,ymode=log,legend pos=south west]
\addplot+[mark=*] table[col sep=comma,x=frequency,y=recovery]{figdata/wide32768_recovery_dense_4.csv};\addlegendentry{$r=4$}
\addplot+[mark=square*] table[col sep=comma,x=frequency,y=recovery]{figdata/wide32768_recovery_dense_8.csv};\addlegendentry{$r=8$}
\addplot+[mark=triangle*] table[col sep=comma,x=frequency,y=recovery]{figdata/wide32768_recovery_dense_full.csv};\addlegendentry{full}
\nextgroupplot[xlabel={step},ylabel={sparse test MSE},xmode=log,ymode=log,legend pos=north east]
\addplot+[mark=none,thick] table[col sep=comma,x=step,y=test_mse]{figdata/wide32768_curve_sparse_4.csv};\addlegendentry{$r=4$}
\addplot+[mark=none,thick] table[col sep=comma,x=step,y=test_mse]{figdata/wide32768_curve_sparse_8.csv};\addlegendentry{$r=8$}
...
\nextgroupplot[xlabel={step},ylabel={dense test MSE},xmode=log,ymode=log,legend pos=north east]
\addplot+[mark=none,thick] table[col sep=comma,x=step,y=test_mse]{figdata/wide32768_curve_dense_4.csv};\addlegendentry{$r=4$}
\addplot+[mark=none,thick] table[col sep=comma,x=step,y=test_mse]{figdata/wide32768_curve_dense_8.csv};\addlegendentry{$r=8$}
...
\item The optimal rank is intermediate.  Ranks $1$--$2$ are too restrictive; full rank is too spectrally biased at finite training time; rank $4$ balances approximation and optimization in our width-$2^{15}$ Fourier sweep.
\end{itemize}
