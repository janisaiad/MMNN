...
def plot_main_figures():
    ...
    fig,axs=plt.subplots(1,4,figsize=(13.2,2.75))
    for col,task in enumerate(['sparse','dense']):
        rec_ax=axs[2*col]
        ...
        for label,style in [('4','-'),('8','-.'),('full','--')]:
            ...
            rec_ax.loglog(xrec,yrec,style,lw=2.2,label=label_fr)
            cur=read_dicts(f'wide32768_curve_{task}_{label}.csv')
            train_label='full' if label=='full' else f'r={label}'
            train_ax.loglog(as_float(cur,'step'),as_float(cur,'excess_test_mse'),style,lw=2.2,label=train_label)
        task_fr='sparse mixture' if task=='sparse' else 'dense mixture'
        ...
        train_ax.set_ylabel('normalized excess MSE')
        train_ax.set_ylim(7e-5, 1.4)
        for ax in (rec_ax, train_ax):
            ax.grid(alpha=.22)
            ax.legend(frameon=False, fontsize=7.5, handlelength=1.8)
            ax.tick_params(labelsize=8)
    for idx,ax in enumerate(axs.ravel()): add_panel_label(ax, chr(65+idx))
    fig.tight_layout(w_pad=0.65); save_figure(fig, 'recovery_training')
