...
\noindent
We collect the standing assumptions and conventions used throughout the paper; expanded discussion is in Appendix~\ref{appendix:notation-assumptions}.
\begin{itemize}
    \item \textbf{Sequential infinite width.} We take \( \bm{n}_1\to\infty,\dots,\bm{n}_L\to\infty \) with i.i.d.\ Gaussian random features, frozen after initialization, under NTK scaling \(1/\sqrt{\bm{n}_\ell}\)~\cite{jacot2018neural} (Appendix~\ref{appendix:assumptions-expanded}).
    \item \textbf{Bottleneck regime and scaling.} For \(\ell\ge 2\), \(\bm{d}_\ell=r\ll \bm{n}_\ell\), and we use edge-of-chaos scaling \( \mathrm{Cov}(w^{(\ell)}_j)=I_r/r \) (Appendix~\ref{appendix:eoc-scaling}).
    \item \textbf{Input normalization.} For RKHS statements we focus on normalized inputs (typically \(x\in\mathbb{S}^{d-1}\)), so kernels are zonal functions of the cosine similarity \(\rho\); concentration statements keep explicit \(\|x\|\|x'\|\) factors when needed.
\end{itemize}

\paragraph{Homogeneous activation.}
We work under the positive 1-homogeneity assumption (satisfied by \(\ReLU\)); see Assumption~\ref{assump:homogeneous_activation} in Appendix~\ref{appendix:notation-assumptions}. This condition is used for Fisher--Kibble decoupling (Lemma~\ref{lem:fisher-kibble}) and for separating radial and angular fluctuations in the three-layer kernel.

\subsection{Parameterization, signal propagation, and kernels}

We briefly record the scaling conventions used in the kernel calculations; details are in Appendix~\ref{appendix:notation-assumptions}.
\paragraph{EOC scaling.}
We choose the variance \(\sigma_A\) so that the forward variance remains stable across depth (edge of chaos); the resulting recursion for the layerwise variance \(q^{(\ell)}\) and the condition on \(\sigma_A\) are given in Appendix~\ref{appendix:eoc-scaling}.
\paragraph{Bottleneck scaling and the \(1/r\) factor.}
In the bottleneck regime \(\bm{d}_{\ell-1}=r\) for \(\ell\ge 2\), the EOC scaling uses \( \mathrm{Cov}(w)=I_r/r \), which keeps base and derivative kernels \(O(1)\). The RF-LR NTK recursion carries an explicit prefactor \(1/r\) at bottleneck layers; see Appendix~\ref{appendix:eoc-scaling} and Appendix~\ref{appendix:proof-ntk-recursion}.

\vspace{0.5em}
With an abuse of notation, we denote by \( \Sigma^{(\ell)} \) the base kernel and by \( \dot{\Sigma}^{(\ell)} \) the derivative kernel.
