...
We define the RF-LR architecture, the training policy, and the base and derivative kernels used in the NTK recursion.

\subsection{RF-LR architecture}
The layer-wise recurrence is as follows. Let \( \bm{h}^{(0)}(x)=x\in\mathbb{R}^{\bm{d}_0} \). For \( \ell=1,\dots,L \),
\begin{equation}
\label{eq:RF-LR}
\bm{h}^{(\ell)}(x)
...
By rotation invariance of Gaussian weights, these kernels depend on inputs only through the cosine similarity \(\rho=\langle x,x'\rangle/(\|x\|\,\|x'\|)\). We will therefore write \(\Sigma^{(\ell)}(\rho)\) and \(\dot{\Sigma}^{(\ell)}(\rho)\). Given these definitions, we can now state the following composition result.
