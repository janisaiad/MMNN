...
\author{
  Janis Aiad\thanks{First author, work done during a visit at University of Maryland}$^{1}$ \and Haizhao Yang$^{2}$ \and Shijun Zhang$^{3}$ \\
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Architecture of a low-rank random-feature network (RF-LR) with a low-rank intermediate layer. The red dashed connections indicate the low-rank component that reduces dimensionality from $n_1$ to $r \ll n_1$ neurons.
