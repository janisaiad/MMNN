...
We now introduce the random-matrix theory conditions that are necessary for the asymptotic analysis. Those strong assumptions can be removed in future work using the RMT extensive width framework~\cite{benigni2025eigenvaluedistributionneuraltangent}, which establishes RMT limits for Gram spectra in the extensive-width regime (\(n\sim N^2\)) with layers taken in the RMT limit.
...
Benigni and Paquette~\cite{benigni2025eigenvaluedistributionneuraltangent} establish the eigenvalue distribution of the NTK in the RMT limit (extensive-width regime \(N\sim n^2\), layers in the RMT limit) for shallow two-layer networks, showing convergence to a deformed Marchenko--Pastur law with a spike-bulk structure. We extend this analysis to the three-layer case with random features and rank-\( r \) bottlenecks, where the frozen feature weights and low-rank structure modify both the spike location and bulk support.

\paragraph{Mean-zero targets: restriction to \texorpdfstring{\(\mathbf{1}^\perp\)}{1-perp} (removing the constant mode).}
Let \(K\in\mathbb{R}^{n\times n}\) be a symmetric Gram matrix built from a kernel evaluated on a dataset \(\{x_i\}_{i=1}^n\). For squared loss with mean-zero targets (or after removing the constant component of the labels), the dynamics depend only on the action of \(K\) on the mean-zero subspace
\[
\mathbf{1}^\perp=\{v\in\mathbb{R}^n:\langle v,\mathbf{1}\rangle=0\}.
\]
In particular, if the kernel includes an additive constant term (here the \(+1\) in \(K_\infty\)), this contributes a rank-one component aligned with \(\mathbf{1}\), which can create a large top eigenvalue in the \emph{uncentered} spectrum but does not affect the eigenvalues of the restriction \(K|_{\mathbf{1}^\perp}\). Convergence rates for mean-zero targets are governed by the spectrum on \(\mathbf{1}^\perp\).
...
\end{itemize}
Proof: See Appendix~\ref{appendix:proof-spike-bulk} (via~\cite{El_Karoui_2010}). This theorem is stated for the deterministic inner-product kernel matrix built from the limiting kernel function \(K_\infty\); incorporating finite-\(r\) Fisher--Kibble fluctuations or finite-width effects into the limiting ESD would require the Benigni--Paquette framework~\cite{benigni2025eigenvaluedistributionneuraltangent} (RMT limits, extensive-width regime, resolvent replacement and Gaussian equivalence) and is left for future work.
\end{theorem}

\begin{remark}[How to interpret the spike for learning]
The spike in Theorem~\ref{thm:spike-bulk} is a statement about the \emph{uncentered} Gram matrix. When the kernel includes an additive constant term (here the \(+1\) in \(K_\infty\)), the associated rank-one component aligns with \(\mathbf{1}\) and yields \(\lambda_{\max}(\mathbf{M})=\Theta(n)\). This does not by itself imply poor conditioning of the learning problem: for mean-zero labels, the relevant spectrum is that of the restriction \(\mathbf{M}|_{\mathbf{1}^\perp}\), i.e. the bulk/shifted spectrum.
\end{remark}
...
Removing the rank-one constant mode aligned with \(\mathbf{1}\) leaves the spectrum on \(\mathbf{1}^\perp\) unchanged (paragraph above); it does not make the remaining eigenvalue distribution ``less skewed''. In particular, in depth-scaling regimes where the relevant \(\mathbf{1}^\perp\) scale becomes small (e.g.\ when the mean diagonal--off-diagonal gap decays like \(1/(rL)\), as for MLPs at EOC; Theorem~\ref{thm:rflr-depth-scaling-mean}), random fluctuations can drive the smallest eigenvalue on \(\mathbf{1}^\perp\) very close to \(0\) even though the empirical Gram matrix is always positive semidefinite (Remark~\ref{rmk:lambda-min-negative-appendix} in Appendix~\ref{appendix:lambda-min-negative}).
