...
\item The bi-Lipschitz bounds for $H_i$ differences with low-rank structure
\item The structure $B_k^{(i)} = \E_{C_i}[L^{(i)}_{C_i,k}\,\varphi_i'(H_i)\,w_i]$, which gives bounds involving factors of $rK$
\end{itemize}
...
\end{align*}
where the last step uses the Lipschitz property of $\varphi_L$ and the bi-Lipschitz bounds for $H_i$ with low-rank structure.
