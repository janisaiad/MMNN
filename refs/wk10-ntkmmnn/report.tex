...
\begin{itemize}
    ...
    \[ \sigma(x, x') = \mathbb{E}_{\bm{w}, b} \left[ \sigma\left(\bm{w}^\top x + \beta b\right) \sigma\left(\bm{w}^\top x' + \beta b\right) \right] \]
    \[ K_{\text{nngp}}^{(\ell)}(x, x') = \mathbb{E}_{\bm{h}^{(\ell-1)}} \left[ \sigma\left(\bm{h}^{(\ell-1)}(x), \bm{h}^{(\ell-1)}(x')\right) \right] \]
