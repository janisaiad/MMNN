...
\quad
\text{up to the skew-gauge component inside } \operatorname{span}(U).
...
\begin{proof}
For fixed \(U\), the differential is \(dW=\alpha U\,dB\). Hence \(d\mathcal L=\langle H,\alpha U\,dB\rangle=\langle \alpha U^\top H,dB\rangle\), giving \eqref{eq:frozen-left-flow}. If \(B\) is fixed and \(U\) varies on the Stiefel manifold, then \(dW=\alpha\,dU\,B\) and the Euclidean gradient is \(\alpha HB^\top\). Projecting this gradient to the normal component of the Stiefel tangent gives \((I-UU^\top)\alpha HB^\top\), while tangent rotations inside \(\operatorname{span}(U)\) are gauge-coupled with \(B\). Multiplying the resulting \(dU\) by \(B\) gives the displayed normal contribution \eqref{eq:moving-left-flow}. Thus training \(U\) introduces terms depending on both \(H\) and \(B^\top B\), so the tangent kernel contains \(BB^\top\)-weighted and cross \(U/B\) blocks in addition to the fixed-projector \(B\)-block. Differentiating the NTK along gradient flow then creates dNTK terms involving \(\dot U\), \(\dot B\), and products such as \(B^\top B\). These are precisely the moving-subspace interactions excluded by freezing \(U\).
\end{proof}
...
Let \(Z_a\), \(a=1,\ldots,r\), be iid rank-channel atoms with uniformly bounded moments of all orders, and define empirical observables
\[
...
\begin{lemma}[Transport-source summation]
...
\Var(Y_0)=1.
\end{equation}
For a transported off-diagonal leg, \(\lambda=3\). The mixed and NTK-NTK endpoint integrals are
\begin{equation}
...
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{../../data/feynman/true_finite_ntk_cumulants_depth_20_50_100_200/cumulant_ratio_V_same_off_over_eps_L.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{../../data/feynman/true_finite_ntk_cumulants_depth_20_50_100_200/cumulant_ratio_C_same_off_over_eps_L2.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{../../data/feynman/true_finite_ntk_cumulants_depth_20_50_100_200/cumulant_ratio_A_same_off_over_eps_L3.pdf}
\end{minipage}
\caption{Deep finite-network cumulant stress test at depths \(L\in\{20,50,100,200\}\). The experiment is deliberately diagnostic: at very large depths the finite-network scalar-output cumulants become noisy and can show nonmonotone effective behavior, but the plots expose the regime where the polynomial normalization remains informative and where finite-depth effects dominate.}
\label{fig:deep-cumulant-stress}
\end{figure}

\begin{table}[t]
\centering
\caption{Exact full-rank matching for true empirical NTKs at \(n=r=128\), \(m=8\), \(d=16\). Differences are at numerical precision, confirming Theorem \ref{thm:pathwise}.}
...
\section{Limitations}
The theory is for signed/isometric low-rank factors and RF-LR bottlenecks, not strict nonnegative matrix factorization. The constants \(5\), \(21/20\), and \(173/720\) are isolated proxy/tensor constants, not direct scalar-output cumulant constants. The operator concentration theorem keeps logarithmic factors and a well-spread data distinction; removing these losses is open. A complete numerical table for all dNTK/ddNTK descendant coefficients is left for future work.
...
\section{Mathematical Claim Ledger}
The following ledger records the status of the mathematical ingredients used in the paper.
\begin{center}
\begin{tabular}{p{0.28\linewidth}p{0.62\linewidth}}
\toprule
Status & Ingredients \\
\midrule
Proved algebraically & Pathwise full-rank matching; fixed-left gradient decomposition; Haar covariance; rank cumulant power counting; endpoint moment calculation; three-layer delta-method expansion. \\
Conditional theorem/proposition & Tensor hierarchy; operator concentration; RF-LR spectral preservation; centered \(L^2\) rule after radial projection. \\
Experimental diagnostic & Direct finite-network cumulant slopes and normalized ratios; operator proxy ratios; finite-depth effective constants outside isolated tensor coordinates. \\
Excluded from the claims & Strictly positive NMF treated as signed Gaussian low rank; training the left factor \(U\) while using fixed-projector diagrammatics. \\
\bottomrule
\end{tabular}
\end{center}
