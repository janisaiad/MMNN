...
\title{Feynman Diagrams and Scaling Laws for\\
Low-Rank Multilayer Neural Networks}
...
\begin{equation}
h_\ell(x)=A_\ell\sigma(W_\ell h_{\ell-1}(x)+b_\ell),
\qquad W_\ell,b_\ell\ \text{frozen},\quad A_\ell\ \text{trainable},
\label{eq:mmnn-block}
\end{equation}
with component dimension \(r_\ell\ll n_\ell\) \citep{MMNN2024}.  Between two nonlinearities the effective matrix is
\(W_{\ell+1}A_\ell\): a frozen left factor followed by a trainable right factor.  Hence a depth-\(L\) MMNN is a concatenation of the fixed-left/trainable-right two-layer objects for which our rank-state Feynman calculus closes.  A QR decomposition of Gaussian \(W_{\ell+1}\) separates its radial Wishart part from a Haar-Stiefel orientation; equation \eqref{eq:intro-model} is exact for the whitened orientation, or equivalently after using the induced metric on \(A_\ell\).  An unwhitened MMNN has additional fixed radial/Wishart vertices and is not pathwise identical to the Stiefel model.  A strict nonnegative factorization changes the tangent cone and is not claimed here.
...
\item For power-law data spectra, we prove a stable-head/random-tail transition, compute the block-Haar tail learning curve exactly by orthogonal Weingarten calculus, and derive the early-stopping protection law \(r\gtrsim mL^3(P\beta)^{2(2+\alpha)/(1+\alpha)}\).
\item We validate the theory with proxy experiments, true finite-network autodiff NTKs, exact Haar moment tests, spectral eigenvector mixing, data-scaling curves, and early-stopping shifts.
\end{itemize}
...
MMNNs compose shallow random-feature maps whose inner weights are fixed and whose component-mixing matrices are trained \citep{MMNN2024}.  Their adjacent product \(W_{\ell+1}A_\ell\) is the motivating fixed-left/trainable-right low-rank matrix.  Benigni and Paquette characterize the limiting NTK eigenvalue distribution of a two-layer network in a quadratic sample--width regime using free multiplicative convolution \citep{BenigniPaquette2025}; our object is complementary, because we concatenate such low-rank blocks and retain finite-width connected corrections.  For generalization, Kramp, Lindner, and Helias derive power-law learning and early-stopping laws by dynamical mean-field theory \citep{KrampLindnerHelias2026}.  We quantify when the finite-rank Feynman perturbation preserves those laws and when near-degenerate spectral eigenvectors randomize.
...
\paragraph{The exponents in the published plots.}
Table \ref{tab:published-effective-exponents} records every critical ReLU power fitted in the vector figures of Guillen, Misof, and Gerken \citep{FeynmanNTK2026}.  Their networks have depths only up to \(30\), and the regressions begin between layers \(5\) and \(12\), so these are effective slopes
\(p_{\rm eff}(L)=\dd\log|T_L|/\dd\log L\), not new diagrammatic degrees.  The thirty fits cluster by sector: \(V_4\) around the asymptotic degree one, \(D,F\) around degree two, and \(A,B\) around degree three.  Off-diagonal components approach later because the ReLU correlation satisfies \(1-\rho_L=O(L^{-2})\), while its derivative transport has \(O(L^{-1}\log L)\) corrections.  If
\(T_L=cL^p(1+aL^{-\omega}+o(L^{-\omega}))\), then
\begin{equation}
p_{\rm eff}(L)=p-\omega aL^{-\omega}+o(L^{-\omega});
\label{eq:effective-slope-drift}
\end{equation}
...
\begin{table}[t]
\centering
\caption{Componentwise critical-depth exponents read from the regression labels in the five vector plots of Guillen, Misof, and Gerken \citep{FeynmanNTK2026}.  The last row is the leading degree predicted by the transport diagrams.}
\label{tab:published-effective-exponents}
\small
...
This closes the \(O_s\) hierarchy of Dyer and Gur-Ari \citep{DyerGurAri2019} under the low-rank rules.  If \((\lambda_i,e_i)\) are eigenpairs of the infinite-width training NTK and \(\Delta f_0\) is the initial residual, their late-time first NTK correction at a test point can be written
\begin{align}
\Theta_\infty^{(1)}(x)
={}&-\sum_i\frac{(O_3(x;0)\cdot e_i)(\Delta f_0\cdot e_i)}{\lambda_i}
\nonumber\\
&+\sum_{i,j}\frac{(e_i^\top O_4(x;0)e_j)
(\Delta f_0\cdot e_i)(\Delta f_0\cdot e_j)}
{\lambda_i(\lambda_i+\lambda_j)}.
\label{eq:O3-O4-correction}
\end{align}
Here \(O_3\) and \(O_4\) are contractions of the dNTK and ddNTK descendants.  Proposition \ref{prop:no-new-vertices} implies that equation \eqref{eq:O3-O4-correction} remains valid for frozen-left low rank after replacing the dense descendant coefficients by their rank-state/projector diagrams.  The small eigenvalue denominators also explain why eigenvector mixing in a power-law tail is the correct observable for the first dynamical correction.

\section{Power-Law Data, Spectral Mixing, and Early Stopping}
\label{sec:powerlaw}
Let the population covariance be
\begin{equation}
\Lambda=\diag(\eta_1,\ldots,\eta_N),
\qquad \eta_j=\eta_1j^{-(1+\alpha)},\quad \alpha>0,
\label{eq:powerlaw-spectrum}
\end{equation}
and write the finite NTK training operator as \(\wh K=\Lambda+E\), with
\(\|E\|_{\op}\le s\).  Theorem \ref{thm:operator} supplies the diagrammatic choice
\begin{equation}
s=s_{L,n,r}:=
C\left[L^{3/2}\sqrt{m\barE}+mL^3\barE\right]
\operatorname{polylog}(m,L,1/\delta).
\label{eq:diagrammatic-s}
\end{equation}
The adjacent population gap is
\begin{equation}
g_j:=\eta_j-\eta_{j+1}
=(1+\alpha)\eta_1j^{-(2+\alpha)}(1+O(j^{-1})).
\label{eq:powerlaw-gap}
\end{equation}

\begin{theorem}[Stable head and mixing threshold]
\label{thm:stable-head}
Let \(q_j\) be the eigenvector of \(\wh K\) associated by eigenvalue order with the population eigenvector \(e_j\), and set \(g_0=+\infty\).  For \(1\le j<N\), whenever
\(\delta_j:=\min(g_{j-1},g_j)>2s\),
\begin{equation}
|\wh\eta_j-\eta_j|\le s,
\qquad
\sin\angle(q_j,e_j)\le \frac{s}{\delta_j-s}.
\label{eq:davis-kahan-head}
\end{equation}
...
\label{eq:Jmix}
\end{equation}
The bound makes no distributional claim about modes \(j\gtrsim J_{\rm mix}\); it only identifies the tail in which Wigner-like mixing is no longer forbidden by the gaps.
\end{theorem}

\begin{proof}
The eigenvalue statement is Weyl's inequality.  The eigenvector statement is the one-dimensional Davis--Kahan theorem, using the perturbed separation at least \(\delta_j-s\).  Substitution of \eqref{eq:powerlaw-gap} into \(g_j\simeq2s\) gives \eqref{eq:Jmix}.
\end{proof}

We next compute the risk if this unresolved tail is orthogonally invariant.  For sample size \(P\), inverse noise temperature \(\beta\), time \(t\), and training eigenvalue \(\kappa\ge0\), define
\begin{equation}
c_t(\kappa)=\frac12e^{-2Pt\kappa}
+\frac{1-e^{-2Pt\kappa}}{2P\beta\kappa},
\label{eq:spectral-cost}
\end{equation}
with its continuous extension at \(\kappa=0\).  The first term is teacher-averaged bias and the second is the Langevin variance in the power-law regression model of Kramp, Lindner, and Helias \citep{KrampLindnerHelias2026}.  If
\(\wh K=Q\diag(\kappa_i)Q^\top\), the exact linearized risk is
\begin{equation}
\mathcal L(t;Q)=\tr\!\left[\Lambda Q\diag(c_t(\kappa_i))Q^\top\right].
\label{eq:exact-linear-risk}
\end{equation}

\begin{theorem}[Exact orthogonal-Weingarten tail risk]
\label{thm:weingarten-risk}
Fix a head size \(0\le J\le N-2\).  Let \(Q=I_J\oplus Q_T\), where \(Q_T\) is Haar on \(O(M)\), \(M=N-J\), and condition on the ordered training eigenvalues \(\kappa_i\).  Write
\begin{align}
S_\eta&=\sum_{i>J}\eta_i,&
S_c&=\sum_{i>J}c_t(\kappa_i),\nonumber\\
S_{\eta,0}^2&=\sum_{i>J}\eta_i^2-\frac{S_\eta^2}{M},&
S_{c,0}^2&=\sum_{i>J}c_t(\kappa_i)^2-\frac{S_c^2}{M}.
\end{align}
Then
\begin{align}
\E_{Q_T}\mathcal L(t;Q)
&=\sum_{i\le J}\eta_i c_t(\kappa_i)+\frac{S_\eta S_c}{M},
\label{eq:weingarten-risk-mean}\\
\Var_{Q_T}\mathcal L(t;Q)
&=\frac{2S_{\eta,0}^2S_{c,0}^2}{(M-1)(M+2)}.
\label{eq:weingarten-risk-var}
\end{align}
These formulas are finite-\(N\) and exact.
\end{theorem}

\begin{proof}
Apply the first and fourth orthogonal Haar moments to
\(\tr(AQ_TBQ_T^\top)\), with
\(A=\diag(\eta_{J+1:N})\) and
\(B=\diag(c_t(\kappa_{J+1:N}))\).  The complete invariant calculation is given in Appendix \ref{app:powerlaw-proofs}.
\end{proof}

\begin{corollary}[Why a fully Haar NTK is the wrong model]
\label{cor:full-haar-wrong}
If \(J=0\), the mean risk is \(N^{-1}(\sum_i\eta_i)(\sum_i c_t(\kappa_i))\); the mode-by-mode pairing of data eigenvalues and learning rates is lost.  Thus a fully Haar eigenbasis generally destroys the aligned power-law scaling.  Equations \eqref{eq:davis-kahan-head}--\eqref{eq:weingarten-risk-var} justify Haar randomization only inside the unresolved tail.
\end{corollary}

The aligned dynamics learns modes satisfying \(P\eta_jt\gtrsim1\), so its moving cutoff is
\(J(t)\asymp(P\eta_1t)^{1/(1+\alpha)}\).  Equating it with \eqref{eq:Jmix} gives
\begin{equation}
t_{\rm mix}
\asymp\frac1{P\eta_1}
\left(\frac{(1+\alpha)\eta_1}{2s}\right)^{(1+\alpha)/(2+\alpha)}.
\label{eq:tmix}
\end{equation}
...
\begin{corollary}[Rank sufficient to protect early stopping]
\label{cor:early-stopping-rank}
The aligned power-law optimum is
\begin{equation}
t_0^*=\frac{\beta}{2}\frac{\alpha}{1+\alpha}
\label{eq:aligned-optimal-time}
\end{equation}
to leading order \citep{KrampLindnerHelias2026}.  A sufficient condition for \(t_0^*\lesssim t_{\rm mix}\) is
\begin{equation}
\frac{s}{\eta_1}\lesssim C_\alpha
(P\beta\eta_1)^{-(2+\alpha)/(1+\alpha)}.
\label{eq:early-stop-s-condition}
\end{equation}
In the low-rank dominated regime \(\barE\asymp r^{-1}\), the variance term of \eqref{eq:diagrammatic-s} therefore yields
\begin{equation}
r\gtrsim
mL^3(P\beta\eta_1)^{2(2+\alpha)/(1+\alpha)}
\operatorname{polylog}(m,L,P,1/\delta).
\label{eq:powerlaw-rank-law}
\end{equation}
The bias term gives the weaker power
\(r\gtrsim mL^3(P\beta\eta_1)^{(2+\alpha)/(1+\alpha)}\) when \(P\beta\eta_1\gtrsim1\).
\end{corollary}

\begin{proof}
Insert \eqref{eq:aligned-optimal-time} into \(t_0^*\lesssim t_{\rm mix}\), rearrange, and then substitute \(s\asymp L^{3/2}\sqrt{m/r}\) or \(s\asymp mL^3/r\) from \eqref{eq:diagrammatic-s}.
\end{proof}

\section{Experiments}
We run four classes of experiments and present the figures in the same order as the theory. Proxy experiments first validate the rank-projector law and operator normalization. We then check the endpoint tensor constants and RF-LR memory suppression. True finite-network experiments compute empirical NTKs by autodiff and estimate direct finite-network covariances of the NNGP Gram and empirical NTK.  Finally, power-law experiments test the exact Weingarten moments, the stable-head threshold, and the predicted early-stopping failure mode.

For the last group we use \(\alpha=1/2\), \(\eta_1=\beta=1\), and \(N=128\) modes for the Wigner deformation
\begin{equation}
\wh K=(\Lambda^{1/2}+\tau W)(\Lambda^{1/2}+\tau W)^\top,
\qquad W\ \text{GOE-normalized},
\label{eq:psd-wigner-experiment}
\end{equation}
which is positive semidefinite while having a Wigner-like leading correction.  At \(\tau=0.02\), the measured perturbation norm is \(s=0.02178\); both the exact gap count and equation \eqref{eq:Jmix} give \(J_{\rm mix}=4\).  Figure \ref{fig:weingarten-spectral} shows that these first modes retain large population overlap while the tail fluctuates near the Haar scale.  In a separate block-Haar test with head size \(32\), \(500\) orthogonal samples give empirical mean \(0.36330037\) and standard deviation \(2.249\times10^{-5}\), versus the exact values \(0.36330004\) and \(2.187\times10^{-5}\) from Theorem \ref{thm:weingarten-risk}.  The mean discrepancy is \(0.34\) Monte--Carlo standard errors.

\begin{figure}[t]
\centering
\begin{minipage}{0.49\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/powerlaw_early_stopping/spectral_eigenvector_mixing.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.49\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/powerlaw_early_stopping/weingarten_risk_distribution.pdf}
\end{minipage}
\caption{Spectral mixing and exact orthogonal moments.  Left: population-eigenvector overlap under the PSD Wigner deformation; the exact gap threshold and the asymptotic power-law estimate coincide at \(J_{\rm mix}=4\).  Right: the block-Haar test-risk distribution with the exact finite-dimensional Weingarten mean and one-standard-deviation band.}
\label{fig:weingarten-spectral}
\end{figure}

To isolate data scaling, we use \(N=8192\) exact power-law modes and replace only the tail beyond \(J_{\rm mix}\) by a Haar block.  The aligned minimum follows
\(P^{-\alpha/(1+\alpha)}=P^{-1/3}\) and its numerical optimum \(t^*=0.173\)--\(0.182\) is close to \(t_0^*=1/6\).  Once the learned cutoff enters the mixed tail, the minimum-risk curve bends above that frontier and the optimum moves earlier.  For example, at \(P=1024\), perturbation scales \(s=0.0025,0.0075,0.02\) give \(J_{\rm mix}=10,6,4\) and optimal-time ratios \(t^*/t_0^*=0.183,0.116,0.077\), respectively.  Figure \ref{fig:powerlaw-scaling-stopping} is an exact evaluation of equations \eqref{eq:weingarten-risk-mean}--\eqref{eq:weingarten-risk-var}, not a fitted surrogate.

\begin{figure}[t]
\centering
\begin{minipage}{0.49\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/powerlaw_early_stopping/minimum_risk_data_scaling.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.49\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/powerlaw_early_stopping/optimal_stopping_data_scaling.pdf}
\end{minipage}
\caption{Consequences of a Haar-mixed power-law tail.  Left: the aligned \(P^{-1/3}\) risk frontier persists until the training cutoff reaches the tail, after which it bends toward a perturbation-controlled plateau.  Right: tail mixing advances the optimal stopping time, increasingly so as \(P\) resolves deeper modes.}
\label{fig:powerlaw-scaling-stopping}
\end{figure}
...
\centering
\includegraphics[width=\linewidth]{figures/lowrank_ntk_proxy_outputs_clean/rank_defect_gamma.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/lowrank_ntk_proxy_outputs_clean/haar_covariance.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/lowrank_ntk_proxy_outputs_clean/operator_proxy_ratio.pdf}
\end{minipage}
...
\centering
\includegraphics[width=\linewidth]{figures/lowrank_ntk_proxy_outputs_clean/convergence_V.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.24\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/lowrank_ntk_proxy_outputs_clean/convergence_C.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.24\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/lowrank_ntk_proxy_outputs_clean/convergence_A.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.24\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/lowrank_ntk_proxy_outputs_clean/rflr_memory_suppression.pdf}
\end{minipage}
...
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_outputs_clean/true_finite_ntk_operator_ratio.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_outputs_clean/true_finite_ntk_rank_defect_scaling.pdf}
\end{minipage}
...
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_cumulants_very_long/cumulant_ratio_V_same_off_over_eps_L.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_cumulants_very_long/cumulant_ratio_C_same_off_over_eps_L2.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_cumulants_very_long/cumulant_ratio_A_same_off_over_eps_L3.pdf}
\end{minipage}
...
\newblock \emph{ICML}, 2026; arXiv:2508.11522.
...
\begin{proposition}[Combined full-width/rank power counting]
\label{prop:combined-power-counting}
Consider a correlation function with \(m_\Gamma\) derivative-tensor vertices and Dyer--Gur-Ari cluster graph having \(n_e\) even components and \(n_o\) odd components.  Assume its dense channel diagrams obey the cluster-graph bound
\begin{equation}
s_C=n_e+\frac{n_o}{2}-\frac{m_\Gamma}{2}.
\label{eq:dyer-cluster-exponent}
\end{equation}
For a low-rank diagram \(\Gamma\), let \(\mathcal R(\Gamma)\) be its empirical-rank vertices, with valence \(d_v\), and \(\mathcal P(\Gamma)\) its centered projector vertices.  At fixed depth,
\begin{equation}
|\mathcal A_\Gamma|
\le C_\Gamma n^{s_C}
\prod_{v\in\mathcal R(\Gamma)}r_{\ell(v)}^{1-d_v}
\prod_{v\in\mathcal P(\Gamma)}
\left\|\cumul_{d_v}(G^{(\ell(v))})\right\|,
\label{eq:combined-power-counting}
\end{equation}
times the deterministic depth transports of Theorem \ref{thm:lowrank-feynman-rules}.  A second projector vertex contributes \(\gamma_{n_\ell,r_\ell}\); more generally a connected valence-\(d\) rank source is \(O(r_\ell^{1-d})\).  At \(r_\ell=n_\ell\), every centered projector diagram is zero and the surviving diagrams are exactly the dense ones.
\end{proposition}

\begin{proof}
Before the low-rank averages are taken, the dense Gaussian part has the same neural-index loops and the same \(n^{-m_\Gamma/2}\) normalization as in the cluster-graph expansion.  The number of loops is at most \(n_e+n_o/2\), giving \(n^{s_C}\).  Conditional independence across rank channels forces all legs at a connected empirical-rank vertex to share one channel index; summing it gives \(r\), while its \(d_v\) normalized legs give \(r^{-d_v}\).  Frozen-projector blocks are independent across layers and contribute their connected cumulants.  Multiplication yields \eqref{eq:combined-power-counting}; deterministic propagation does not change width/rank counting.  Finally \(G=I\) at \(r=n\), so all its centered cumulants vanish, while Theorem \ref{thm:pathwise} identifies the remaining diagrams with the dense expansion.
\end{proof}
...
\section{Proofs for Power-Law Spectral Mixing}
\label{app:powerlaw-proofs}

\subsection{Aligned learning curve and optimal time}
Let \(q=1+\alpha\), \(a=\alpha/(1+\alpha)\), and
\(p=1/(1+\alpha)\).  For an aligned kernel \(\wh K=\Lambda\), equation \eqref{eq:exact-linear-risk} separates as
\begin{align}
\mathcal L_{\rm bias}(t)
&=\frac12\sum_{j=1}^N\eta_j e^{-2Pt\eta_j},
\label{eq:appendix-aligned-bias}\\
\mathcal L_{\rm var}(t)
&=\frac1{2P\beta}\sum_{j=1}^N(1-e^{-2Pt\eta_j}).
\label{eq:appendix-aligned-var}
\end{align}
In the scaling window \(1\ll2Pt\eta_1\ll N^{1+\alpha}\), Euler--Maclaurin summation gives
\begin{align}
\mathcal L_{\rm bias}(t)
&=\frac{\eta_1\Gamma(a)}{2(1+\alpha)}
(2Pt\eta_1)^{-a}(1+o(1)),
\label{eq:appendix-bias-asymptotic}\\
\mathcal L_{\rm var}(t)
&=\frac{\Gamma(a)}{2P\beta}
(2Pt\eta_1)^p(1+o(1)).
\label{eq:appendix-var-asymptotic}
\end{align}
Indeed, with \(u=2Pt\eta_1x^{-q}\),
\begin{align}
\int_0^\infty \eta_1x^{-q}e^{-2Pt\eta_1x^{-q}}\,\dd x
&=\frac{\eta_1}{q}(2Pt\eta_1)^{-a}\Gamma(a),\\
\int_0^\infty(1-e^{-2Pt\eta_1x^{-q}})\,\dd x
&=(2Pt\eta_1)^p\Gamma(a).
\end{align}
Differentiating \eqref{eq:appendix-bias-asymptotic}--\eqref{eq:appendix-var-asymptotic} yields
\(2Pt\eta_1=aP\beta\eta_1\), hence
\begin{equation}
t_0^*=\frac{a\beta}{2}
=\frac{\beta}{2}\frac{\alpha}{1+\alpha},
\qquad
\mathcal L(t_0^*)\asymp
\eta_1^{1/(1+\alpha)}(P\beta)^{-\alpha/(1+\alpha)}.
\label{eq:appendix-aligned-optimum}
\end{equation}
This proves the aligned scaling statements used in Section \ref{sec:powerlaw}; finite-\(N\) tail subtraction produces the familiar \(O(N^{-\alpha})\) correction.

\subsection{Orthogonal fourth moment}
We prove Theorem \ref{thm:weingarten-risk}.  The following identity is the finite-dimensional orthogonal Weingarten calculation behind it.

\begin{lemma}[Orthogonal twirl]
\label{lem:orthogonal-twirl}
Let \(Q\) be Haar on \(O(M)\), \(M\ge2\), and let \(A,B\) be deterministic real symmetric matrices.  With
\(A_0=A-(\tr A/M)I\) and \(B_0=B-(\tr B/M)I\),
\begin{align}
\E\tr(AQBQ^\top)&=\frac{\tr A\,\tr B}{M},
\label{eq:appendix-twirl-mean}\\
\Var\tr(AQBQ^\top)&=
\frac{2\tr(A_0^2)\tr(B_0^2)}{(M-1)(M+2)}.
\label{eq:appendix-twirl-var}
\end{align}
\end{lemma}

\begin{proof}
Haar invariance implies \(\E[QBQ^\top]=(\tr B/M)I\), proving the mean.  Constant trace components have no variance, so it remains to consider \(A_0,B_0\) in the space \(\mathsf S_0^M\) of traceless symmetric matrices.  The orthogonal action \(B_0\mapsto QB_0Q^\top\) is irreducible on this space.  Therefore its invariant covariance is
\begin{equation}
\E\!\left[\langle A_0,QB_0Q^\top\rangle_F^2\right]
=c_B\|A_0\|_F^2.
\end{equation}
Summing this identity over an orthonormal basis of \(\mathsf S_0^M\) gives
\(\|B_0\|_F^2=c_B\dim(\mathsf S_0^M)\).  Since
\(\dim(\mathsf S_0^M)=M(M+1)/2-1=(M-1)(M+2)/2\), equation \eqref{eq:appendix-twirl-var} follows.  This is the invariant form of the orthogonal fourth-moment/Weingarten contraction \citep{CollinsSniady2006}.
\end{proof}

Set \(A=\diag(\eta_{J+1:N})\),
\(B=\diag(c_t(\kappa_{J+1:N}))\), and add the deterministic aligned-head contribution.  Then
\(\tr(A_0^2)=S_{\eta,0}^2\) and
\(\tr(B_0^2)=S_{c,0}^2\), so Lemma \ref{lem:orthogonal-twirl} gives equations \eqref{eq:weingarten-risk-mean}--\eqref{eq:weingarten-risk-var}.

\subsection{Reproducibility}
The power-law figures and all plotted CSV values are generated by
\texttt{experiments/feynman/run\_powerlaw\_early\_stopping.py} with seed \(20260806\), \(N=128\), \(\alpha=1/2\), \(\beta=1\), \(P=64\), \(16\) Wigner repetitions, \(500\) Weingarten repetitions, and an \(8192\)-mode exact block-Haar scaling calculation.  The automated tests in \texttt{tests/test\_powerlaw\_weingarten.py} check the aligned risk identity, the orthogonal-twirl degeneracy for a constant spectrum, and the zero-variance block-Haar limit at \(t=0\).
...
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/lowrank_ntk_proxy_outputs_clean/haar_covariance_relative_error.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
...
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/lowrank_ntk_proxy_outputs_clean/loglog_slopes_scalar_modes.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
...
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_cumulants_very_long/cumulant_loglog_V_same_off.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_cumulants_very_long/cumulant_loglog_C_same_off.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_cumulants_very_long/cumulant_loglog_A_same_off.pdf}
\end{minipage}
...
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_cumulants_depth_20_50_100_200/cumulant_loglog_V_same_off.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_cumulants_depth_20_50_100_200/cumulant_loglog_C_same_off.pdf}
\end{minipage}
\hfill
\begin{minipage}{0.32\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/true_finite_ntk_cumulants_depth_20_50_100_200/cumulant_loglog_A_same_off.pdf}
\end{minipage}
