...
\icmltitlerunning{Low-Rank Neural Networks and Finite-Width NTK}

\begin{document}

\twocolumn[
\icmltitle{Low-Rank Neural Networks and Finite-Width NTK\\
...
\icmlkeywords{low-rank neural networks, neural tangent kernel, finite width, feature learning}
\vskip 0.3in
]
...
Trainable low-rank, \(r=32\) & 440,362 & 98.30 \\
\bottomrule
\end{tabular}
\end{table}
...
\end{equation}
If \(U\) is also trained on the Stiefel manifold, the normal projected component contributes, up to rotations inside \(\operatorname{span}(U)\),
\begin{equation}
\dot W_U
...
\begin{aligned}
\Sigma^{(\ell)}(x,x')
&:=\E[\sigma(g_x^{(\ell)})\sigma(g_{x'}^{(\ell)})],\\
\dot\Sigma^{(\ell)}(x,x')
&:=\E[\sigma'(g_x^{(\ell)})\sigma'(g_{x'}^{(\ell)})].
\end{aligned}
...
\end{align}
The normalized cumulant-ratio panels are reported in Appendix \ref{app:extended-experiments}; the main text keeps the log-log plots because they show the effective depth exponents directly.
...
\begin{equation}
\begin{aligned}
\|\mathcal S_V^{(\ell)}\|&\le C,\\
\|\mathcal S_C^{(\ell)}\|&\le C(1+\ell+\|\mathcal V_\ell\|),\\
\|\mathcal S_A^{(\ell)}\|
&\le C(1+\ell^2+\ell\|\mathcal V_\ell\|+\|\mathcal C_\ell\|
+\|\mathcal V_\ell\|^2).
\end{aligned}
...
\begin{tabular}{rrrr}
\toprule
\(r\) & \(V_{\rm dir}\) & \(C_{\rm dir}\) & \(A_{\rm dir}\) \\
\midrule
8 & \(2.01\) & \(2.03\) & \(2.06\) \\
16 & \(1.76\) & \(1.79\) & \(1.81\) \\
32 & \(1.79\) & \(1.81\) & \(1.83\) \\
...
\(V(e,f)\) & \(5\) \\
\(D,F\), diag. leg & \(B_0=3\) \\
\(D,F\), off-diag. leg & \(B_3=21/20\) \\
\(A,B\), diag./diag. & \(A_{0,0}=7/3\) \\
\(A,B\), diag./off-diag. & \(A_{0,3}=43/60\) \\
\(A,B\), off/off & \(A_{3,3}=173/720\) \\
\bottomrule
\end{tabular}
\end{center}
The proxy convergence panels and RF-LR memory plot are deferred to Appendix \ref{app:extended-experiments}.
...
The main text keeps the log-log cumulant panels and effective slope table, since these directly display the observed depth exponents. Appendix \ref{app:extended-experiments} reports the non-log-log proxy panels, normalized cumulant ratios, remaining true finite-network NTK plots, the deep cumulant stress test, and exact full-rank matching.
...
\bibitem[Arora et al.(2019)]{Arora2019}
S. Arora, S. Du, W. Hu, Z. Li, R. Salakhutdinov, and R. Wang.
\newblock On exact computation with an infinitely wide neural net.
\newblock \emph{NeurIPS}, 2019.

\bibitem[Nguyen and Mondelli(2020)]{NguyenMondelli2020}
...
\bibitem[Nguyen et al.(2021)]{NguyenMondelliMontufar2022}
...
\bibitem[Chizat and Bach(2018)]{ChizatBach2018}
L. Chizat and F. Bach.
\newblock On the global convergence of gradient descent for over-parameterized models using optimal transport.
\newblock \emph{NeurIPS}, 2018.

\bibitem[Rotskoff and Vanden-Eijnden(2018)]{RotskoffVandenEijnden2018}
...
\subsection{Proxy and normalized diagnostics}
The following non-log-log panels support the constants and normalizations used in the main text. They are kept here so the main ICML narrative can focus on the depth-exponent evidence.
