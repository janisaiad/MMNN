...
\caption{Rank-shift control. The best rank is task-dependent: broadening the target spectrum or increasing the finite-budget weight on high frequencies moves the optimum from the baseline value to much larger ranks, up to about $20$ or $40$ in the strongest controls. Horizontal dashed colored lines show the corresponding full-rank MSE baselines for each setting. The Sobolev-weighted setting means that the loss gives larger weight to high-frequency modes, as in Sobolev or Fourier training objectives; this makes additional rank useful for recovering oscillatory components.}
...
\caption{Mode-wise recovery and training curves. The recovery ratio is $R_r(k,t)=|\widehat f_{r,t}(k)|/|\widehat f^\star(k)|$: values near one mean that a Fourier mode is recovered, while values near zero mean that it is missing. The legend reports the fitted log-log recovery slope $\beta$ for each displayed curve. A steep negative slope indicates that high frequencies are much less recovered than low frequencies. The theory predicts the direction of these finite-resolution slopes through the face moments $M_q$ and spectral transfer coefficients, but not a universal numerical slope independent of task, budget, and frequency window.}
...
\label{eq:rank-risk}
\end{equation}
Here $A r^{-2s}$ is the approximation term: if the target has effective smoothness or compressibility $s$, increasing rank reduces the error made by forcing the representation through an $r$-dimensional bottleneck. Thus very small ranks are bad because this term is large. The term $B d_{\rm eff}(r)/n$ is the statistical or effective-dimension cost: $d_{\rm eff}(r)$ counts the number of degrees of freedom effectively used at rank $r$, so this term grows when rank makes the model too flexible for the sample size $n$. The last term is the finite-time spectral-transfer error. Each Fourier mode $k$ of the target has energy $|\wh f^\star(k)|^2$ and is learned at rate $\lambda_r(k)$; after time $t$, the residual contribution of that mode is weighted by $e^{-2t\lambda_r(k)}$. A useful rank is therefore not the smallest rank or the largest rank: it is the first rank for which the approximation term is already controlled, while the spectral rates $\lambda_r(k)$ remain balanced enough that high frequencies are not much slower than low frequencies.
