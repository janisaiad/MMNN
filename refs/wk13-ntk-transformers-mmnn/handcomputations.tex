...
\boxed{
\begin{aligned}
\Theta^{(2)}(\rho, \rho_1) &= \Theta^{(1)}(\rho) \cdot \sigma_A^2 \left(1 - \frac{\arccos(\rho_1)}{\pi}\right) + \Theta^{(1)}(\rho_1)\|x_1\| \|y_1\|  + 1 \\[1em]
\text{with} \quad \Theta^{(1)}(\rho) &=  \frac{1}{\pi}\left(\sqrt{1-\rho^2} + \rho(1-\arccos(\rho))\right)
\end{aligned}
}
\end{center}
...
The squared norms $x_1^2$ and $y_1^2$ follow a bivariate Kibble distribution and their cosine distance follows an independent Fisher like distribution.
...
We can then state the final result: the NTK for a 2-layer network with ReLU activation is given by:

\begin{center}

    For 2 entries x,y in $\mathbb{R}^d$ and a low rank r hidden layer, the NTK for a 2 hidden layer MMNN is :
    \newline
\boxed{
\begin{aligned}
\Theta^{(2)}(x,y) &= \Theta^{(1)}(\rho) \cdot \sigma_A^2 \left(1 - \frac{\arccos(\rho_1)}{\pi}\right) + \Theta^{(1)}(\rho_1)\|x_1\| \|y_1\|  + 1 \\[1em]
\text{with} \quad \Theta^{(1)}(\rho) &=  \frac{1}{\pi}\left(\sqrt{1-\rho^2} + \rho(1-\arccos(\rho))\right)
\end{aligned}
}
\end{center}
...
\subsection{Distribution of the Correlation Coefficient $r$ (derived by Fisher)}
\paragraph{Distribution} The exact law of $r$ (cosine of the angle between two Gaussian vectors of dimension $n$ with correlation $\rho$) was first derived by R. A. Fisher in his seminal 1915 paper "Frequency Distribution of the Values of the Correlation Coefficient in Samples from an Indefinitely Large Population" published in \textit{Biometrika}. The modern form of this distribution is given by:
\[
...
\paragraph{Curse of Dimensionality} The variance of $r$ decreases as $O(1/n)$, not exponentially. When $\rho$ is of order $O(1/\sqrt{n})$, the correlation signal becomes overwhelmed by geometric noise.

\subsection{The Kibble Distribution for the Squared Norms $(U,V)$}
\paragraph{Distribution} The joint law of the correlated variables $U=\|X\|^2 \sim \chi^2(n)$ and $V=\|Y\|^2 \sim \chi^2(n)$ follows the bivariate Kibble-Gamma distribution, first described by W. F. Kibble in his 1941 paper "A Two-Variate Gamma Type Distribution" in \textit{Sankhyā}. The distribution takes the form:
\[
...
\paragraph{Curse of Dimensionality - A Misnomer for Correlated Variables}
The behavior of these variables is quite interesting in high dimensions. The expectation of $U$ is $n$ with variance $2n$, showing linear growth with dimension. The coefficient of variation $CV = \sqrt{2n}/n = \sqrt{2}/\sqrt{n}$ approaches zero, indicating the distribution becomes relatively tighter around its mean. When normalized, $U/n$ has mean 1 and variance $2/n$, converging to the constant 1 following the Law of Large Numbers, with a $\mathcal{N}(1, 2/n)$ approximation from the Central Limit Theorem. Notably, this concentration occurs polynomially in $n$, not exponentially.

\subsection{Product of Kibble Variables $W=UV$ - Breaking the Curse}
The product of Kibble variables exhibits complex behavior that can be expressed through infinite series or special functions like Meijer-G or Whittaker functions. The mean $\E[W] = n^2 + 2n\rho^2$ demonstrates quadratic growth with additional correlation effects. The variance shows strong correlation dependence: $\Var(W) = (4n^3 + 4n^2) + (4n^3 + 32n^2 + 32n)\rho^2 + (4n^2 + 16n)\rho^4$. For the normalized product $W/n^2$, the variance is approximately:
\[
...
\]
A crucial insight is that when $\rho \neq 0$, the variance decays only polynomially as $O(1/n)$, not exponentially. The correlation $\rho$ serves as a delocalization factor preventing extreme concentration.
...
The Fisher transformation and the angle $\theta=\arccos(r)$ are related via the Gudermannian function, though this relationship serves more as a mathematical curiosity than a practical tool:
\[
