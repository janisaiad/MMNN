...
\boxed{
\begin{aligned}
\Theta^{(2)}(\rho, \rho_1) &= \Theta^{(1)}(\rho) \cdot \sigma_A^2 \operatorname{Tr}(\Sigma_w)  \left(\frac{1}{2} - \frac{\arccos(\rho)}{2\pi}\right) + \Theta^{(1)}(\rho_1)\|x_1\| \|y_1\|  + 1 \\[1em]
...
\boxed{
\begin{aligned}
\Theta^{(2)}(x,y) &= \Theta^{(1)}(\rho) \cdot \left(1 - \frac{\arccos(\rho_1)}{\pi}\right)\frac{1}{r}  + \Theta^{(1)}(\rho_1)\|x_1\| \|y_1\| + 1 \\[1em]
\text{with} \quad \Theta^{(1)}(\rho) &= \frac{1}{\pi}\left(\sqrt{1-\rho^2} + \rho(1-\arccos(\rho))\right)
\end{aligned}
}
\end{center}
...
Fisher derived the exact law of $r$ in 1915, representing the cosine between Gaussian vectors of dimension $n$ with correlation $\rho$:
...
For large $n$, Fisher's z-transform gives $z = \text{arctanh}(r) \sim \mathcal{N}\left(\text{arctanh}(\rho) + \frac{\rho}{2(n-1)}, \frac{1}{n-3}\right)$. The variance of $r$ decreases as $O(1/n)$. When $\rho \sim O(1/\sqrt{n})$, geometric noise masks correlation.
...
Mean $\E[U] = n$ and variance $\Var(U) = 2n$ grow linearly. $U/n$ converges to $\mathcal{N}(1, 2/n)$ with polynomial concentration.

\subsection{Product of Kibble Variables}
The product $W=UV$ has mean $\E[W] = n^2 + 2n\rho^2$. Its variance shows correlation dependence:
...
For $\rho \neq 0$, variance decays as $O(1/n)$, with $\rho$ preventing extreme concentration.

\subsection{Independence Properties}
The orientation variable $r$ is independent of magnitudes $(U,V)$. Fisher transform and angle $\theta=\arccos(r)$ connect via Gudermannian:
