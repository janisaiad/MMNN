...
Recent empirical work by 
 ...
 RF-LRs achieve \textbf{global strong-convergence} across a wide range of tasks, including neural-network based PDE solving with and without Physics-Informed losses~\cite{raissi2017physicsinformeddeeplearning}. 
 Training exhibits \textbf{super convergence} in 1st order optimization and a distinctive \textbf{stepwise loss dynamics}, where long plateaus are punctuated by sharp drops.
...
\noindent
\begin{itemize}
\item \emph{How NTK analysis for RF-LR can explains their 1st order super-convergence properties ?}
\end{itemize}
\vspace{0.5em}
   ...
   fundamentally alter the kernel's spectral properties. A key technical advantage of our approach is that we \textbf{avoid computing correlation diagrams} by
    ...
    analysis while maintaining rigor. Parallel to this, random feature approximations~\cite{rahimi2007random,rahimi2008weighted} provide a practical route to kernel
     approximation by sampling features from suitable distributions, with rigorous guarantees for generalization~\cite{bach2017breaking,rudi2017generalization} and
      ...
      at all finite times—a property stronger than classical kernel ridge assumptions.
...
On the optimization side, gradient descent near sharp minima can be unstable~\cite{cohen2021gradient}, with edge-of-stability (EOS) phenomena causing loss oscillations
 when eigenvalues exceed \( 2/\eta \). Different optimizers exhibit distinct implicit biases~\cite{wilson2017marginal}, and in sharp regimes switching to SGD can improve
  stability. Neural scaling laws~\cite{kaplan2020scaling,hoffmann2022training} describe test loss as power laws in compute, data, and model size, while emergent 
  capabilities~\cite{wei2022emergent} appear at critical scales. Universal approximation theorems~\cite{cybenko1989approximation,hornik1989multilayer} and random-feature
   universality~\cite{rahimi2008weighted} ensure expressivity, and in three-layer frozen-feature settings, approximation power is maintained at \emph{all} training
    times~\cite{nguyen2021proofglobalconvergencegradient}. Loss landscape geometry~\cite{li2018visualizing,fort2019large} significantly affects trainability; as training progresses, 
    the landscape often convexifies~\cite{chizat2020implicit}, with Hessian spectrum evolution providing quantitative measures of this transition.
...
for quadratic scaling. To ensure the NTK has a positive smallest eigenvalue—crucial for optimization 
 ...
 factors for deep networks, motivating architectures that achieve NTK-like behavior with tractable computational requirements for large scale applications.
