...
\begin{proof}
We prove that under Assumption~\ref{assump:homogeneous_activation}, the mean NTK \( \tilde{\Kop}^{(L)} \) (obtained by taking expectations over Fisher and Kibble distributions) is a zonal kernel that induces the same RKHS as the shallow (two-layer) ReLU kernel.

\paragraph{Setup: Two-layer case.}
...
\]
where \( \hat{\rho}_r \sim \text{Fisher}(\rho, r) \) and \( (\|x_1\|^2, \|y_1\|^2) \sim \text{Kibble}(r, \rho) \) are independent by Lemma~\ref{lem:fisher-kibble}. The mean NTK is:
\[
\tilde{\Kop}^{(2)}(\rho) = \mathbb{E}\!\left[\hat{\Theta}^{(2)}_r(x,x')\right] = \Theta^{(1)}(\rho) \cdot \mathbb{E}\!\left[1 - \frac{\arccos(\hat{\rho}_r)}{\pi}\right] + \frac{1}{r} \cdot \mathbb{E}\!\left[\Sigma^{(1)}(\hat{\rho}_r) \cdot \|x_1\|\|y_1\|\right] + 1,
\]
where expectations are over Fisher and Kibble distributions.

\paragraph{Computation of \( \mathbb{E}[\arccos(\hat{\rho}_r)] \).}
By Fisher's CLT, \( \sqrt{r}(\hat{\rho}_r - \rho) \xrightarrow{d} \mathcal{N}(0, (1-\rho^2)^2) \), so \( \hat{\rho}_r = \rho + O_p(r^{-1/2}) \). For \( \rho \in (-1,1) \), the function \( \arccos \) is smooth, so by Taylor expansion:
\[
\arccos(\hat{\rho}_r) = \arccos(\rho) - \frac{1}{\sqrt{1-\rho^2}}(\hat{\rho}_r - \rho) + O_p((\hat{\rho}_r - \rho)^2).
\]
Taking expectations:
\[
\mathbb{E}[\arccos(\hat{\rho}_r)] = \arccos(\rho) - \frac{1}{\sqrt{1-\rho^2}}\mathbb{E}[\hat{\rho}_r - \rho] + O(r^{-1}) = \arccos(\rho) + O(r^{-1}),
\]
since \( \mathbb{E}[\hat{\rho}_r] = \rho + O(r^{-1}) \) by Fisher's distribution properties. Therefore:
\[
\mathbb{E}\!\left[1 - \frac{\arccos(\hat{\rho}_r)}{\pi}\right] = 1 - \frac{\arccos(\rho)}{\pi} + O(r^{-1}).
\]

\paragraph{Computation of \( \mathbb{E}[\Sigma^{(1)}(\hat{\rho}_r) \cdot \|x_1\|\|y_1\|] \).}
By independence of Fisher and Kibble (Lemma~\ref{lem:fisher-kibble}), and using \( \mathbb{E}[\|x_1\|\|y_1\|] = r + O(1) \) from Kibble's distribution:
\[
\mathbb{E}\!\left[\Sigma^{(1)}(\hat{\rho}_r) \cdot \|x_1\|\|y_1\|\right] = \mathbb{E}[\Sigma^{(1)}(\hat{\rho}_r)] \cdot \mathbb{E}[\|x_1\|\|y_1\|] + \text{Cov}(\Sigma^{(1)}(\hat{\rho}_r), \|x_1\|\|y_1\|).
\]
Since \( \Sigma^{(1)} \) is smooth and \( \hat{\rho}_r = \rho + O_p(r^{-1/2}) \), we have \( \mathbb{E}[\Sigma^{(1)}(\hat{\rho}_r)] = \Sigma^{(1)}(\rho) + O(r^{-1}) \). The covariance term is \( O(r^{-1/2}) \) by concentration. Therefore:
\[
\frac{1}{r} \cdot \mathbb{E}\!\left[\Sigma^{(1)}(\hat{\rho}_r) \cdot \|x_1\|\|y_1\|\right] = \Sigma^{(1)}(\rho) + O(r^{-1/2}).
\]

\paragraph{Mean NTK expression.}
Combining the above:
\[
\tilde{\Kop}^{(2)}(\rho) = \Theta^{(1)}(\rho)\!\left(1 - \frac{\arccos(\rho)}{\pi}\right) + \Sigma^{(1)}(\rho) + 1 + O(r^{-1/2}).
\]
As \( r \to \infty \), this converges to \( K_{\infty}(\rho) \), but for finite \( r \), the mean NTK has corrections.

\paragraph{Puiseux expansion of mean NTK near \( \rho = 1 \): Full Fisher density computation.}
...
For the hypergeometric function near its argument \( 1 - (t+v)/2 \to 1^- \), we use the connection formula and asymptotic expansion:
\[
...
By analyzing the asymptotics of \( I(r) \) as \( r \to \infty \), we find \( I(r) = 1 + O(r^{-1}) \), yielding:
\[
\mathbb{E}[\arccos(\hat{\rho}_r)] = \sqrt{2t} + O(t^{3/2}) + O(r^{-1} t^{1/2}).
\]

Therefore, the mean NTK expansion is:
\[
\tilde{\Kop}^{(2)}(1-t) = \Theta^{(1)}(1-t) \cdot \left(1 - \frac{\mathbb{E}[\arccos(\hat{\rho}_r)]}{\pi}\right) + \cdots = 1 - \frac{2}{\pi} t^{1/2} + O(t^{3/2}) + O(r^{-1} t^{1/2}),
\]
which has the same leading-order \( t^{1/2} \) term as the deterministic kernel \( K_{\infty}(\rho) \), ensuring the same spectral decay rate and RKHS structure. The \( O(r^{-1} t^{1/2}) \) correction from the full Fisher density is subdominant to the \( O(t^{3/2}) \) term for \( t \gg r^{-1} \).
