...
We can now state the main result that the mean NTK \(\tilde{\Kop}^{(L)}\) induced has same RKHS decay as the shallow ReLU kernel.
...
Recall that for layer \( \ell \), the base kernel is
\[
\Sigma^{(\ell)}(x,x') = \mathbb{E}_{\bm{w}}\Big[\sigma\!\big(\bm{w}^\top \bm{h}^{(\ell-1)}(x)\big)\,\sigma\!\big(\bm{w}^\top \bm{h}^{(\ell-1)}(x')\big)\Big],
\]
and the derivative kernel is
\[
\dot{\Sigma}^{(\ell)}(x,x') = \mathbb{E}_{\bm{w}}\Big[\dot{\sigma}\!\big(\bm{w}^\top \bm{h}^{(\ell-1)}(x)\big)\,\dot{\sigma}\!\big(\bm{w}^\top \bm{h}^{(\ell-1)}(x')\big)\;\bm{w}^\top\bm{w}\Big].
\]

For \( \ell = 1 \) and centered Gaussian \( \bm{w} \sim \mathcal{N}(0, \mathbf{I}_d) \), these kernels are rotation-invariant and admit closed forms in terms of input cosine similarity \( \rho = \langle x,x'\rangle/(\|x\|\|x'\|) \). For ReLU activation, following~\cite{bietti2021deepequalsshallowrelu}, we have the standard formulas:
\[
\Sigma^{(1)}(x,x') = \frac{\|x\|\|x'\|}{2\pi}\big((\pi-\theta)\cos\theta + \sin\theta\big), \quad \theta = \arccos(\rho),
\]
and for the derivative kernel:
\[
\dot{\Sigma}^{(1)}(x,x') = \frac{d}{2\pi}(\pi - \theta), \quad \theta = \arccos(\rho),
\]
where \( d \) is the input dimension and the factor comes from \( \mathbb{E}[\|\bm{w}\|^2] = d \) for isotropic Gaussian weights. These formulas are the same as in~\cite{bietti2021deepequalsshallowrelu}.

From these closed forms, we can derive the Puiseux series expansions near \( \rho = 1 \) (for \( t \ge 0 \)):
\[
\Sigma^{(1)}(1-t) = 1 - \frac{2}{\pi} t^{1/2} + O(t^{3/2}),\qquad
\dot{\Sigma}^{(1)}(1-t) = \frac{d}{2} - \frac{d}{2\pi} t^{1/2} + O(t^{3/2}).
\]

For the mean NTK \( \tilde{\Kop}^{(2)}(\rho) = K_{\infty}(\rho) \), the Puiseux expansion near \( \rho = 1 \) follows from combining the base and derivative kernels. We can now directly apply Theorem 1 of~\cite{bietti2021deepequalsshallowrelu} to obtain the spectral decay characterization:

\begin{theorem}[Spectral decay for RF-LR NTK on \( \mathbb{S}^{d-1} \)]\label{cor:spectral_decay_bietti}
Following~\cite{bietti2021deepequalsshallowrelu}, let \( \kappa:[-1,1]\to\mathbb{R} \) be the rotation-invariant kernel associated to the (deep or shallow) ReLU NTK on \( \mathbb{S}^{d-1} \). Assume \( \kappa \in C^\infty(-1,1) \) and admits the asymptotic expansions, for \( t\ge 0 \),
\[
\kappa(1-t) = p_1(t) + c_1 t^{\nu} + o(t^{\nu}),\qquad
\kappa(-1+t) = p_{-1}(t) + c_{-1} t^{\nu} + o(t^{\nu}),
\]
where \( p_1,p_{-1} \) are polynomials, \( \nu>0 \) non-integer, and derivatives satisfy analogous expansions. Then by Theorem 1 of~\cite{bietti2021deepequalsshallowrelu}, there exists a constant \( C(d,\nu)>0 \) such that the eigenvalues \( \mu_k \) (in the spherical harmonics basis) satisfy:
\begin{itemize}
    \item For even \(k\) and \( c_1 \neq -c_{-1}\): \( \mu_k \sim (c_1+c_{-1})\,C(d,\nu)\,k^{-d-2\nu+1} \).
    \item For odd \(k\) and \( c_1 \neq c_{-1}\): \( \mu_k \sim (c_1-c_{-1})\,C(d,\nu)\,k^{-d-2\nu+1} \).
\end{itemize}
If \( |c_1|=|c_{-1}| \), then \( \mu_k = o\big(k^{-d-2\nu+1}\big) \) for one parity (or both if \(c_1=c_{-1}=0\)). If \( \kappa \) is \( C^\infty \) on \([{-}1,1]\) with no such \( \nu \), then \( \mu_k \) decays faster than any polynomial.
\end{theorem}

\begin{corollary}[No advantage in RKHS]\label{thm:no_rkhs_advantage}
Under the RF-LR setting with ReLU nonlinearity, isotropic random features on the sphere \( \mathbb{S}^{d-1} \), and in the kernel (NTK) regime, the depth-\(L\) NTK induces the \emph{same} RKHS as the shallow (two-layer) ReLU kernel. In particular, the corresponding RKHSs coincide as sets with equivalent norms. This is a direct application of Theorem 1 in~\cite{bietti2021deepequalsshallowrelu}.
\end{corollary}
