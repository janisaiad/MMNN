...
Having established concentration results, we now turn to the mean NTK. The key observation is that the empirical NTK depends on random variables that follow Fisher and Kibble distributions. The mean NTK is obtained by taking expectations over these distributions.
...
\]
where \( \hat{\rho}_r \) follows Fisher's distribution (centered at \( \rho \)) and \( \|x_1\|\|y_1\| \) follows Kibble's distribution. The mean NTK is therefore:
\[
...
\]
where the expectations are over Fisher and Kibble distributions. This yields a zonal kernel with a Puiseux expansion that differs from the deterministic kernel \( K_{\infty}(\rho) \), but preserves the same endpoint behavior, ensuring RKHS equivalence.
\end{corollary}

Proof: See Appendix~\ref{appendix:proof-conditional-rkhs-homogeneous}.

\begin{remark}
This corollary is highly non-trivial: we must compute expectations of \( \arccos(\hat{\rho}_r) \) where \( \hat{\rho}_r \sim \text{Fisher}(\rho,r) \), and expectations of products involving Kibble-distributed norm products. The resulting mean NTK has a different Puiseux expansion than the deterministic kernel, but the endpoint behavior (which determines spectral decay and RKHS structure) is preserved under homogeneity, ensuring RKHS equivalence.
\end{remark}
