...
From the proof, near \( \rho = 1 \) (for \( t \ge 0 \)), the mean two-layer NTK admits the Puiseux expansion:
\[
\tilde{\Kop}^{(2)}(1-t) = 1 - \frac{2}{\pi} t^{1/2} + O(t^{3/2}) + O(r^{-1} t^{1/2}),
\]
where the leading non-polynomial term has exponent \( \nu = 1/2 \) with coefficient \( c_1 = -2/\pi \). Similarly, near \( \rho = -1 \), by rotational symmetry and homogeneity of ReLU, the expansion has the same structure with \( c_{-1} = c_1 = -2/\pi \) (see~\cite{bietti2021deepequalsshallowrelu}).
