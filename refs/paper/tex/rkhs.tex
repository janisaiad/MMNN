...
\]
For RF-LR with frozen random features, the RKHS is characterized as follows. Universal approximation holds due to full support of \( w^{(\ell)} \) (frozen Gaussian draws), so any function in the Barron space can be approximated with error \( O(1/\sqrt{n}) \). The rank-\( r \) bottleneck acts as a spectral filter that removes directions with tiny eigenvalues and improves generalization by implicit regularization. Finally, for large \( r \), the RF-LR RKHS is \emph{nearly identical} to the MLP RKHS, implying no expressivity loss, while the parameter budget drops from \( O(n^2) \) to \( O(rn) \).

\vspace{0.5em}

\begin{theorem}[No advantage in RKHS]\label{thm:no_rkhs_advantage}
...
\end{corollary}
\begin{remark}
Corollary~\ref{cor:spectral_decay_bietti} supplies explicit rates for approximation and generalization in the RF-LR NTK RKHS and justifies that deeper NTKs do not enlarge the RKHS beyond the shallow ReLU case (Theorem~\ref{thm:no_rkhs_advantage}).
\end{remark}
...
Under Assumption~\ref{assump:homogeneous_activation}, let \( x_1, y_1 \in \mathbb{R}^r \) be the rank-\( r \) Gaussian projections of inputs \( x, y \), with arbitrary fixed correlation \( \rho \in [-1,1] \). Define
\[
W \;=\; \frac{\|x_1\|\,\|y_1\|}{r}.
\]
