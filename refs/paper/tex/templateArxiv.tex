...
\begin{itemize}
    \item \textbf{Q1.} How does band-by-band Fourier mode activation in mean-field kernel gradient flow explain the observed stepwise loss curves in MMNN training?
    \item \textbf{Q2.} How does the universal approximation property holding at all finite times (from three-layer mean-field theory) ensure global convergence despite non-convexity?
    \item \textbf{Q3.} How do low-rank bottlenecks of rank \( r \) shape NTK fluctuations (\( O(1/r) \) or \( O(1/r^2) \)) and induce scaling laws in the extensive-rank regime \( r \asymp d^\beta \)?
    \item \textbf{Q4.} How does depth correlate with maximal learned frequency, and why do we observe \( 2n \) spikes for \( 2n \) layers?
\end{itemize}
...
At initialization, the MMNN NTK exhibits poor conditioning, explaining the difficulty of SGD in early training. We analyze the spectrum via:
\begin{itemize}
    \item \textbf{Lyapunov product expansion:} The recursive NTK structure yields a product of \( (1 - \arccos(\rho_\ell)/\pi) \) factors across layers, where \( \rho_\ell \) is the layer-\( \ell \) cosine similarity. This product concentrates around small values at initialization, yielding a flat spectrum with tiny outliers.
    \item \textbf{Random matrix perturbations:} The rank-\( r \) bottleneck induces random matrix perturbations to the base kernel. Using results from~\cite{spectrum_kernel_random_matrices}, we characterize bulk and outlier behavior: the bulk follows a Marchenko-Pastur law (when \( N/d = O(1) \)), and a single large outlier emerges controlling early training speed.
    \item \textbf{Comparison with MLPs:} Using Terjék-style analysis, we show that MMNN NTK outliers decay slower than MLP outliers due to the \( O(1/r) \) expansion, yielding improved conditioning for moderate \( r \approx 20{-}30 \).
\end{itemize}
...
In the extensive-rank regime \( r \asymp d^\beta \) for \( \beta \in (0,1) \), the training dynamics exhibit emergent (staircase-like) behavior:
\begin{itemize}
    \item Each plateau corresponds to a subset of latent directions (multi-index model) being inactive.
    \item Each sharp drop corresponds to activating a new direction (or frequency band in Fourier space).
    \item The number of accessible directions scales as \( O(r) \), consistent with the rank bottleneck.
\end{itemize}

\vspace{1em}
\subsection{Band-by-band Fourier mode activation}
We propose that the stepwise loss curve arises from sequential activation of Fourier modes:
\begin{enumerate}
    \item \textbf{Initial phase} (\( t \lesssim t_1 \)): Low frequencies \( |\omega| \lesssim \omega_1 \) are learned via large \( \kappa_t(\omega) \). Loss drops rapidly as \( \mathcal{L}(t) \sim \int_{|\omega| \lesssim \omega_1} |\hat{e}_0(\omega)|^2 e^{-2\int_0^t \kappa_s(\omega) ds} d\omega \to 0 \).
    \item \textbf{First plateau} (\( t \approx t_1 \)): Residual energy sits in \( \omega_1 < |\omega| < \omega_2 \), where \( \kappa_t(\omega) \approx 0 \). No learning occurs; loss is flat.
    \item \textbf{Second phase} (\( t \gtrsim t_1 \)): Mean-field transport of \( A^{(\ell)} \) causes \( \kappa_t(\omega) \) to grow on \( \omega_1 < |\omega| < \omega_2 \). New modes activate; loss drops again.
    \item This process repeats across \( O(r) \) bands, each corresponding to a latent direction in the extensive-rank additive model.
\end{enumerate}
...
\]
where \( v_j \in \mathbb{R}^d \) are latent directions and \( g_j : \mathbb{R} \to \mathbb{R} \) are ridge functions. Classical multi-index learning exhibits staircase behavior because directions \( v_j \) are learned sequentially~\cite{benarous2021online}. In MMNN:
\begin{itemize}
    \item Frozen random features \( w^{(\ell)} \) provide a dense span approximating all possible directions \( v_j \).
    \item The low-rank bottleneck \( r \) limits how many directions can be simultaneously active.
    \item Training greedily activates directions in order of decreasing correlation with the residual.
\end{itemize}
...
In contrast, for 1D functions \( f : [0, 2\pi] \to \mathbb{R} \), MMNN learns \emph{localized high-frequency features directly}:
\begin{itemize}
    \item Instead of learning a "direction" (which is trivial in 1D), the network learns \emph{spatial localization}—where in the domain \( [0, 2\pi] \) each high-frequency spike is placed.
    \item Each frozen random feature \( \ReLU(w_j x + b_j) \) naturally provides spatial localization via the bias \( b_j \), which determines the spike location.
    \item The trainable coefficients \( A_j \) select which locations are active and with what amplitude, directly constructing localized bumps/spikes.
    \item High frequencies are built by \emph{combining localized spikes} across layers, not by projecting onto global Fourier modes.
\end{itemize}
...
Empirical observations reveal that MMNN learns \emph{dictionary functions} or \emph{wavelets}:
\begin{itemize}
    \item \textbf{Spike localization:} ReLU gating selects intervals where spikes occur. The learned functions exhibit sharp, localized spikes rather than smooth low-frequency components.
    \item \textbf{Recursive frequency building:} Each layer builds higher frequencies by combining spikes from the previous layer. Depth correlates directly with maximal learned frequency: deeper networks achieve higher \( \omega_{\max} \).
    \item \textbf{Observation:} \( 2n \) layers yield approximately \( 2n \) distinct spikes in the learned function, suggesting a recursive doubling mechanism.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Early/mid training:} Adam with adaptive learning rates handles the poorly conditioned early landscape and navigates the mean-field plateaus effectively.
    \item \textbf{Edge of stability (EOS):} At certain critical times, Adam becomes unstable due to large eigenvalues of the Hessian. Loss wobbles; training stagnates.
    \item \textbf{Switch to SGD:} Switching to SGD with small learning rate stabilizes training. SGD is better at learning the final high-frequency spikes (sharp, localized features) after the global structure is established.
\end{itemize}
...
SGD with constant learning rate \( \eta \) and batch size \( B \) exhibits implicit regularization toward flatter minima:
\begin{itemize}
    \item The injected noise scales as \( \sigma_{\text{noise}}^2 \propto \eta^2 / B \).
    \item At equilibrium (if SGD converges to a stationary distribution), the effective "temperature" \( T \propto \eta / B \) biases sampling toward wider basins.
    \item Sharp minima require exponentially precise parameter values; noise makes them entropically unfavorable.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Learning rate schedules:} When using Adam, decay the learning rate aggressively in late training to reduce effective temperature and prevent escape.
    \item \textbf{Optimizer switching:} Switch from Adam to SGD when entering the sharp regime (quantified by \( \lambda_{\max}(H) > \text{threshold} \)).
    \item \textbf{Batch size tuning:} Larger batch sizes reduce noise, improving stability near sharp minima but potentially slowing convergence in early training.
    \item \textbf{Monitoring sharpness:} Track \( \lambda_{\max}(H_t) \) and \( \text{tr}(H_t) / \text{tr}(G_t) \) as early indicators of when to adjust the optimization strategy.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Experiment 1:} Cosine regression on \( [0, 2\pi] \) with varying \( d, r, n \). We measure NTK eigenvalue spectrum at initialization and compare to theoretical predictions (Marchenko-Pastur bulk + single outlier).
    \item \textbf{Experiment 2:} Fourier decomposition of learned functions at \( t = t_{\text{early}} \). We confirm that only low frequencies \( |\omega| < \omega_0 \) are learned, with \( \omega_0 \) predicted by the NTK symbol \( \kappa_0(\omega) \).
    \item \textbf{Experiment 3:} Inductive bias validation: targets with low-frequency content are learned exponentially faster than high-frequency targets.
    \item \textbf{Experiment 3b (TO BE COMPLETED):} Two-layer width scheme investigation. For 2-layer MMNN (\( L=2 \)), we systematically vary width configurations \( (n_1, n_2) \) and rank \( r \) to test finite-width NTK predictions. We compare: (i) theoretical NTK from Theorem~\ref{thm:finite_width_ntk} with finite-width corrections \( O(1/n) + O(1/r) \), (ii) empirical NTK computed via finite differences at initialization, and (iii) actual training dynamics deviation from kernel regression predictions. Width schemes tested: \( n_1, n_2 \in \{128, 256, 512, 1024, 2048\} \) with \( r \in \{5, 10, 20, 30, 50\} \). We measure: (a) relative error \( \|\Kop_{\text{empirical}} - \Kop_{\text{theory}}\|_F / \|\Kop_{\text{theory}}\|_F \), (b) deviation from kernel regime measured by \( \|f_t - f_{\text{NTK}}(t)\|_{L^2} \) where \( f_{\text{NTK}}(t) \) solves the linearized kernel regression, (c) variance of NTK entries across random initializations to verify concentration rates. This validates the \( O(1/r^2) \) concentration bound (doubly-random Gram regime) and identifies the minimal \( (n, r) \) for accurate kernel predictions.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Experiment 4:} Training on synthetic multi-index targets \( f^*(x) = \sum_{j=1}^r g_j(\langle v_j, x \rangle) \). We track loss curves and measure the number of stepwise drops vs. \( r \), confirming \( O(r) \) scaling.
    \item \textbf{Experiment 5:} Fourier energy distribution over time. We plot \( \|\hat{f}_t\|_{L^2(|\omega| \in [\omega_k, \omega_{k+1}])}^2 \) for frequency bands \( k = 1, \dots, K \), showing sequential activation (band \( k \) energy drops only after band \( k{-}1 \) is learned).
    \item \textbf{Experiment 6:} Dictionary learning visualization. We plot the partial functions \( \phi_j(x) = A_j \ReLU(\langle w_j, x \rangle) \) and show spike localization and recursive frequency building across layers.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Experiment 7:} Hessian spectrum through training. We compute \( \lambda_{\max}(H_t) \) and \( \text{tr}(H_t)/\text{tr}(G_t) \) at regular intervals, showing the sharpness transition at late training.
    \item \textbf{Experiment 8:} Optimizer switching ablation. We compare (i) Adam only, (ii) SGD only, (iii) Adam \( \to \) SGD switching at various times. We measure final loss, training time, and generalization gap.
    \item \textbf{Experiment 9:} Depth vs. frequency. We train networks with \( L = 2, 3, 4, 5, 6 \) layers on the same target and measure \( \omega_{\max} \) (maximal learned frequency). We confirm \( \omega_{\max} \propto L \) and observe \( 2L \) spikes for \( L \) layers.
\end{itemize}
...
We can tune the frequency learning behavior by adjusting:
\begin{itemize}
    \item \textbf{Variance of \( w \) at initialization:} Larger \( \sigma_w^2 \) increases the frequency cutoff \( \omega_0 \), allowing the network to learn higher frequencies earlier in training.
    \item \textbf{muP parameterization:} Adjusting the maximal update parameter (muP) controls the effective kernel scale, shifting the spectral bias and enabling learning of different frequency ranges.
\end{itemize}
...
The learned dictionary functions exhibit two distinct behaviors depending on the training phase:
\begin{itemize}
    \item \textbf{Interval selection:} Many learned basis functions resemble the ReLU activation structure \( \Phi(x) = \ReLU(\langle w, x \rangle) \) from the MMNN construction. These functions select a range of intervals where they are active, effectively partitioning the input space.
    \item \textbf{Blob placement:} Within each selected interval, the network places localized "blobs" or spikes via thresholding. These blobs are positioned randomly at initialization but become organized during training to match the target function's structure.
\end{itemize}
...
Importantly, this provides a dual perspective on MMNN approximation:
\begin{enumerate}
    \item \textbf{Classical view:} The network approximates the target as \( f(x) \approx \sum_j a_j \phi_j(x) \), where \( \phi_j \) are learned basis functions and \( a_j \) are their coefficients (trained weights \( A^{(\ell)} \)).
    \item \textbf{Dictionary view:} The network decomposes \( f \) into localized features \( f = \sum_j \psi_j \), where each \( \psi_j \) is a blob placed at a specific location and scale, selected from the frozen random feature dictionary.
\end{enumerate}
...
We compare:
\begin{itemize}
    \item \textbf{MMNN:} Low-rank architecture with rank \( r \) bottleneck, parameter budget \( O(rn) \).
    \item \textbf{MLP:} Standard fully-connected network with same total parameter count, parameter budget \( O(n^2) \) but matched to \( O(rn) \) by adjusting width.
\end{itemize}
For fair comparison, we match total trainable parameters and apply identical optimization protocols (Adam with same learning rate schedule, batch size, etc.).
...
\begin{itemize}
    \item \textbf{Theory prediction:} MMNN should exhibit improved concentration with \( O(1/r^2) \) decay (doubly-random Gram regime) vs. MLP's standard \( O(1/N) \) decay. For matched parameter budgets, this implies MMNN reaches the deterministic kernel limit faster.
    \item \textbf{Outlier structure:} Both architectures should show a single dominant outlier controlling early training (Terjék-style), but MMNN's outlier magnitude should be better conditioned due to low-rank structure.
    \item \textbf{Bulk spectrum:} MMNN bulk follows Marchenko-Pastur law with better concentration; MLP bulk is broader with larger fluctuations.
\end{itemize}
...
\begin{itemize}
    \item \textbf{MMNN prediction:} Clear stepwise (staircase) loss behavior with \( O(r) \) distinct drops corresponding to rank-\( r \) bottleneck limiting simultaneous direction learning.
    \item \textbf{MLP prediction:} May exhibit smoother loss curves or less pronounced stepwise structure due to higher-dimensional parameter space allowing more directions to be learned simultaneously.
    \item \textbf{Convergence speed:} MMNN should converge faster in early training (better NTK conditioning) but may show plateaus due to rank bottleneck; MLP may be slower initially but avoid rank-induced plateaus.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Initialization:} Compare \( \lambda_{\max}(H_0) \) and condition number for MMNN vs. MLP.
    \item \textbf{Mid-training:} Compare landscape convexification rates and sharpness transitions.
    \item \textbf{Convergence:} Compare sharpness of final minima and generalization performance.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Hypothesis:} MMNN achieves competitive or superior performance to MLP at \( O(rn) \) parameters vs. MLP at same parameter count.
    \item \textbf{Efficiency metric:} MMNN with \( r \approx 20{-}30 \) should match MLP performance while being \( 10{\times}{-}50{\times} \) more parameter-efficient.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Finite-width corrections:} Extend NTK analysis to finite \( n \) with \( 1/r \) expansion in depth, potentially explaining infinite-depth limits.
    \item \textbf{Quantitative bounds:} Derive rigorous convergence rates for stepwise loss, tightening connections to multi-index learning theory.
    \item \textbf{Tensor programs:} Apply tensor program framework for hyperparameter transfer and feature learning beyond NTK.
    \item \textbf{Feynman diagrams and \( \mu \)-parameterization:} Develop systematic NTK computation rules using Feynman diagram techniques combined with tensor program formalism for \( \mu \)-parameterization. This provides a unified computational framework for deriving global NTK theory across arbitrary depth \( L \), heterogeneous width schemes, and low-rank bottlenecks, systematically analyzing finite-width corrections via diagrammatic expansion orders.
    \item \textbf{PINN and Sobolev training:} Extend to physics-informed neural networks with Fourier-space loss functions to mitigate spectral bias.
    \item \textbf{Transformers and attention:} Adapt low-rank random feature ideas to attention mechanisms and self-attention layers.
    \item \textbf{Benchmark comparison:} Systematic comparison with MLPs on standard benchmarks (ResNets, Vision Transformers) at matched parameter budgets.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Inputs:} \( x,x' \in \mathbb{R}^{d_0} \); cosine similarity \( \rho=\langle x,x'\rangle/(\|x\|\|x'\|) \) when inputs are normalized.
    \item \textbf{Layers:} \( \bm{h}^{(0)}(x)=x \) and \( \bm{h}^{(\ell)}(x) \) denotes the layer-\( \ell \) representation; \( n_\ell \) is the width of layer \( \ell \); \( L \) is the total depth.
    \item \textbf{Activation:} \( \ReLU(\cdot) \); its weak derivative is denoted \( \dot{\ReLU}(\cdot) \) (equals \( \mathbf{1}\{\cdot>0\} \) a.e.).
    \item \textbf{Parameters:} \( \bm{A}^{(\ell)} \in \mathbb{R}^{d_{\ell} \times n_\ell} \) are trainable output weights at layer \( \ell \). In the simplified model, we use unit scaling (no \( \sigma_A, \sigma_c, \beta \)).
    \item \textbf{Random features:} \( \bm{w}^{(\ell)} \) are frozen random parameters at layer \( \ell \); unless stated, \( \bm{w}^{(\ell)} \) are i.i.d. across neurons and layers with centered Gaussian laws. Bias can be absorbed via data augmentation \( x \mapsto [x, 1] \).
    \item \textbf{Kernels:}
    \begin{itemize}
        \item NNGP base kernel at layer \( \ell \): \( \displaystyle \Sigma^{(\ell)}(x,x')=\mathbb{E}_{\bm{w}}\big[\ReLU(\bm{w}^\top \bm{h}^{(\ell-1)}(x))\,\ReLU(\bm{w}^\top \bm{h}^{(\ell-1)}(x'))\big] \).
        \item Derivative kernel: \( \displaystyle \dot{\Sigma}^{(\ell)}(x,x')=\mathbb{E}_{\bm{w}}\big[\dot{\ReLU}(\bm{w}^\top \bm{h}^{(\ell-1)}(x))\,\dot{\ReLU}(\bm{w}^\top \bm{h}^{(\ell-1)}(x'))\,\bm{w}^\top\bm{w}\big] \).
        \item NTK at layer \( \ell \): \( \Kop^{(\ell)}(x,x') \) with recursion \( \Kop^{(\ell)} = \Kop^{(\ell-1)}\cdot\dot{\Sigma}^{(\ell)} + \Sigma^{(\ell)} \).
        \item Fourier symbol: \( \kappa_t(\omega) \) is the time-varying Fourier transform of the kernel operator \( \Theta_t \), diagonalizing the gradient flow in frequency space.
    \end{itemize}
    \item \textbf{Expectations:} \( \mathbb{E}_{\bm{w}}[\cdot] \) indicates expectation over the layer-\( \ell \) random parameters \( \bm{w}^{(\ell)} \); other expectations are indicated explicitly.
    \item \textbf{Limits:} we take widths \( n_1\to\infty,\dots,n_L\to\infty \) \emph{sequentially}; LLN applies conditionally at each step.
    \item \textbf{Low-rank bottleneck:} \( r \) denotes the intermediate rank; kernel fluctuations scale as \( O(1/r) \) or \( O(1/r^2) \) depending on the regime.
    \item \textbf{Extensive-rank regime:} \( r \asymp d^\beta \) for \( \beta \in (0,1) \), motivated by multi-index learning and scaling laws.
    \item \textbf{Mean-field:} \( \rho_t \) denotes the particle distribution of trainable weights; \( R(\rho) \) is the population risk functional.
    \item \textbf{Fourier modes:} \( \hat{f}(\omega) \) is the Fourier transform; \( \hat{e}_t(\omega) = \hat{f}_t(\omega) - \hat{f}^*(\omega) \) is the residual in frequency space.
    \item \textbf{Sharpness:} \( H = \nabla^2 \mathcal{L} \) is the Hessian; \( G = J^\top J \) is the Gauss-Newton matrix; \( \lambda_{\max}(H) \) is the maximum eigenvalue.
    \item \textbf{Operators:} \( \mathbf{I}_d \) is the \( d\times d \) identity; \( \|\cdot\| \) is the Euclidean/operator norm depending on context.
    \item \textbf{Zonal kernels:} "zonal" refers to rotation-invariant kernels on the sphere (dependence on \( \rho \) only).
\end{itemize}
