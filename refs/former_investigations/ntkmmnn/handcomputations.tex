...
\noindent\textbf{Theorem (Two-layer MMNN NTK with low-rank bottleneck).}\label{thm:mmnn-2layer-final}
For two inputs \(x,y\in\mathbb{R}^d\) and a low-rank \( r \) hidden layer, the NTK for a two-hidden-layer MMNN is:
...
\end{itemize}
\noindent\textit{Proof sketch.} Combine the recursive NTK form and the exact ReLU derivative factor with conditioning on the random first-layer representation. Invoke Fisher’s law for \( \rho_1 \) and Kibble’s bivariate chi-square for the norms; angular–radial decoupling yields independence and the stated expression. High-dimensional concentration in \( r \) follows from Gaussian norm concentration. See Appendix (Fisher and Kibble distributions).
