...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/epochs_to_escape_vs_lr.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/epochs_to_escape_vs_bs.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/plot_loss_and_fit_before_after_lr_divides_L2.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/plot_loss_and_fit_before_after_lr_divides_L3.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/plot_partial_before_after_lr_divides_L3.png}
...
\appendix
\section{Appendix}
Config files (e.g.\ \texttt{experiments/table/results\_sumcos\_selected\_rerun/f3\_N768\_bs8\_L2/config.json}) and the training script \texttt{experiments/table/run\_selected\_sumcos\_configs.py} define the full parameterization and LR schedule. Broader baseline sweeps (sumcos rank 5/10, factor 1--5, batch 1--16) and their analysis (worked/failed counts, histograms of min loss per factor) are produced by \texttt{run\_scaling\_law\_depth\_width.py} (e.g.\ \texttt{--baseline-sweep}, \texttt{--baseline-sweep-rank5}) and \texttt{analyze\_baseline\_sweep.py} (\texttt{--sumcos}, \texttt{--sumcos-rank5}); batch-size analysis and run naming are documented in \texttt{experiments/table/BASELINE\_SWEEP\_SETUPS\_AND\_RANKS.md}. Log-ratio trajectory plots (Figures~\ref{fig:logratio_div}--\ref{fig:logratio_conv}) and epochs-to-escape scaling plots (Figures~\ref{fig:escape_vs_lr}, \ref{fig:escape_vs_bs}) come from \texttt{experiments/table/plateau/} (script \texttt{run\_plateau\_escape.py}); scaling law index and batch-size/rank sweep references: \texttt{experiments/table/INDEX\_LOGRATIO\_ESCAPE\_SCALING.md}. Frequency/layer-scaling training and CSV summary by \texttt{--train} and \texttt{--analyze}. Stable baseline (cos $2\pi x$, $N=\mathrm{width}=1024$, rank 10, $L=2$) reaches test error $\sim 1.9{\times}10^{-6}$ (momentum 0.7). Additional details are in \texttt{idea.md} and \texttt{experiments/partialfunctionanalysis/}.
