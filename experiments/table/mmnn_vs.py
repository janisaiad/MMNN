...
class AblationConfig:
    ...
    hidden_width: int = 666  # we set hidden width
...
def generate_ablation_configs() -> List[AblationConfig]:
    ...
    rank_options = [15, None]  # we use None to indicate rank=width
        ...
        for fixWb in fixWb_options:
            for rank_val in rank_options:
                if rank_val is None:  # we use width as rank
                    actual_rank = 666  # we set rank to width
    ...
    with torch.no_grad():
        ...
        if config.use_pinn:  # we check if PINN
            ...
            if len(all_colloc_inputs) > 0:  # we check if we have data
                ...
                pinn_tensors = {}  # we create empty dict
        else:
            pinn_tensors = {}  # we create empty dict
...
def main():
    ...
    print(f"training on device: {device}")  # we print device
