...
# %%
def get_data(net_size,index_weight11,index_weight12,index_weight21,index_weight22):   
    ...
    for i in range(n):
        ...
        for j in range(n):
            model.fcs[fc_idx].weight.data[index_weight11,index_weight12] = X[i,j]
            model.fcs[fc_idx].weight.data[index_weight21,index_weight22] = Y[i,j]
            # print(model.fcs[0].weight.data[0,0])
...
with open(f"figures/sinquad/config_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}_seed{seed}_{act_idx}_{s}_{num_samples}_{W_idx}_{net_size[0]}_{net_size[1]}_{net_size[2]}_{net_size2[0]}_{net_size2[1]}_{net_size2[2]}.json", "w") as f:
    ...
    f.write(f'index_weight11: {index_weight11}')
    f.write(f'index_weight12: {index_weight12}')
    f.write(f'index_weight21: {index_weight21}')
    f.write(f'index_weight22: {index_weight22}')
