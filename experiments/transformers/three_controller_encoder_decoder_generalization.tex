...
$d_i$ on $\operatorname{span}U$ and one on its complement.  Thus it lies in
$(0,I]$.  Finally $H\preceq\operatorname{tr}(H)I=s_H\overline L I$ and
$B_\theta\preceq s_H^{-1}I$ imply
\[
 \lambda_{\max}(B_\theta^{1/2}HB_\theta^{1/2})
 =\lambda_{\max}(H^{1/2}B_\theta H^{1/2})
 \leq s_H^{-1}\lambda_{\max}(H)\leq\overline L.
\]
\end{proof}
...
assume \(A^rY_\theta\) has full column rank.  Define the two small block
moments
\[
...
the projected normal geometry, its spectral correction, and the
preconditioner application are exact algebraic functions of
\((K_r,J_r)\).  A finite-dimensional risk calculation for this head closes
...
Substitution yields \(U^\top AU=C\).  The implementation assigns multiplier
...
Then
\[
 \operatorname{rank}(A_\theta-A)\leq 2S,
 \qquad
 \sup_x\left|F_{A_\theta}(x)-F_A(x)\right|\leq\frac{2S}{K}.
\]
...
\begin{proof}
Write
\(B_\theta^{1/2}=s_H^{-1/2}(I+UEU^\top)\) for the exact \(S\)-dimensional
spectral function \(E\).  Expanding \(A_\theta-A\) shows that its image lies
...
\((K_r,J_r)\) in Proposition~\ref{prop:matrix-free-moment-interface} and of
