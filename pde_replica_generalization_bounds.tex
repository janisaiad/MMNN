...
\subsection{NQF block audit and its boundary}

The attention normal form depends on which parameters are trained near zero
\cite{ziyin2026nqf}.  For a full multi-head term
\[
 v^\top W^V X\operatorname{softmax}
 \left(X^\top(W^K)^\top W^Qx/\sqrt{d_k}\right),
\]
scaling all four query, key, value, and readout blocks by $\varepsilon$
produces a quadratic uniform-average term, whereas prompt-dependent Q/K
routing first appears at quartic order.  Its Q/K loss gradient is therefore
$O(\varepsilon^3)$ for an order-one residual, compared with
$O(\varepsilon)$ for V/readout.

For the query--key-only decoder with fixed nonzero values $c$, the leading
trainable correction is instead
\[
 \frac{\varepsilon^2}{\sqrt{d_k}}
 (\Sigma_Xc)^\top(W^K)^\top W^Qx,
 \qquad
 \Sigma_X=XX^\top/N-\bar x\bar x^\top,
\]
so both Q and K receive $O(\varepsilon)$ gradients.  This is the appropriate
local model for a head that routes fixed weak-form values into preconditioner
directions.  A full MHA with every block small is not equivalent to it.

The NQF order parameter $M=WW^\top$ describes activation of parameter modes
during training.  It is not the PCG residual polynomial, whose modes are those
of the promptwise operator $B_\theta^{1/2}HB_\theta^{1/2}$ during inference.
The generalized Lotka--Volterra reduction additionally assumes commuting
structure matrices (or a specified isotropic case); the GP-forced PDE prompt
law does not establish that condition.  Thus NQF supplies blockwise
initialization predictions and diagnostics, while the HB/Chebyshev/PCG
convergence statements and the replica risk calculation retain their existing
hypotheses.  In particular, once $B_\theta$ is frozen within a solve, the PCG
...
mean relative $H$-risk $9.00\times10^{-4}$ with the original trace-scaled
frozen head.  This improves on
...
A paired rerun freezes the tightly certified preconditioner heads used by the
HB/PCG audit and trains the measure controller for 1600 updates.  Same-head
PCG--8, trained HB--8, and learned-measure Chebyshev--8 respectively attain
$1.8113\times10^{-8}$, $7.3392\times10^{-5}$, and
$1.5091\times10^{-4}$ relative $H$-risk.  The corresponding spectral
prediction is $1.50896\times10^{-4}$, agreeing with the realized Clenshaw
risk, while the prompt-ESD oracle polynomial reaches $1.73\times10^{-8}$ in
float32.  Hence the remaining Chebyshev gap is the estimation of promptwise
spectral masses, not approximation of the hard-coded recurrence.  A residual
guard falls back to PCG on $92.6$--$95.9\%$ of prompts and is not a competitive
...
The same closure now yields a spectral controller without a free measure MLP.
Let \(A_\theta=B_\theta^{1/2}HB_\theta^{1/2}\),
\(\tau_\theta=\operatorname{tr}A_\theta\), and let \(Q\) orthonormalize
\(\{U,A_\theta U,\ldots,A_\theta^{q-1}U\}\), assumed to have dimension
\(qS\).  If \(\vartheta_i\) are the eigenvalues of
\(T_q=Q^\top A_\theta Q\), define
\[
 \bar\vartheta_\perp
 ...
 +(K-qS)\bar\vartheta_\perp\delta_{\bar\vartheta_\perp}\right].
 \label{eq:pde-routed-krylov-measure}
\]
Here
\[
 \tau_\theta
 =\zeta^{-1}\left[\overline L+
 \operatorname{tr}\{(c_\star C^{-1}-I)C\}\right]
\]
is already a deterministic function of the matrix-free block moments.  The
Chebyshev-basis Gram solve applied to \(\widehat\nu_q\) is exact fixed
algebra; only the initial block \(U\) is learned.  If \(qS=K\), this measure
is the exact energy-weighted ESD and gives the exact expected-risk polynomial
under an isotropic coefficient prior.  Otherwise it is an explicit
first-moment complement closure whose replica prediction requires only the
joint law of the existing block moments and the additional finite Krylov
blocks.

On the correlated PDE audit, \(q=2,S=4\) gives
\(9.676\times10^{-6}\) relative \(H\)-risk, compared with
\(7.339\times10^{-5}\) for trained HB and \(1.509\times10^{-4}\) for the
measure MLP.  Fine-tuning the routed head changes this to
\(9.945\times10^{-6}\), providing no gain.  Same-head PCG remains at
\(1.812\times10^{-8}\), and head-free PCG--28 at equal scalar-HVP work gives
\(4.739\times10^{-8}\).  Increasing to \(q=3\) gives
\(8.868\times10^{-6}\), while \(q=4\) is statistically unchanged at
\(8.862\times10^{-6}\).  Thus \(q=2\) is the risk--work knee: the
first-principles measure closes most of the fixed-polynomial estimation gap
but does not reverse the Krylov boundary.

The remaining complement closure can be relaxed without learning any solver
arithmetic.  Keep the eight $q=2$ Ritz nodes and their exact trace, and learn
only three prompt-conditioned statistics: a resolved/complement task-energy
gate, a two-atom complement balance, and a bounded spread.  Parameterizing
the two complement nodes around $\bar\vartheta_\perp$ preserves
\[
 \pi\vartheta_-+(1-\pi)\vartheta_+
 =\bar\vartheta_\perp
\]
exactly, while sigmoid bounds keep both nodes positive and below the certified
spectral upper endpoint.  The learned object is therefore a conditional
closure for missing task-energy information, not an approximation of the
Gram solve or Clenshaw recurrence.

With the covariance head frozen, a width--12 map for these three scalars,
trained for 800 steps over three seeds, lowers relative $H$-risk from
$9.60\times10^{-6}$ at initialization to $4.86\times10^{-7}$.  This is a
factor $19.8$ improvement over the one-atom complement closure and a factor
$4.51$ over pure PCG--8.  It remains a factor $10.3$ worse than head-free PCG
at equal scalar-HVP work ($4.74\times10^{-8}$), and a factor $26.8$ worse
than same-head PCG--8 ($1.81\times10^{-8}$).  Hence the conditional
three-statistic law is empirically real and useful, but it does not overturn
the exact Krylov ordering.
