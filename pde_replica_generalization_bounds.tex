...
\title{Replica Bounds for the PDE Encoder and Linear-Attention Decoder}
\author{}
\date{}
...
The decoder analysis below concerns the collapsed one-shot linear map
$W_{\rm dec}(K/M)\widehat G^\top\widehat b$.  It is not, by itself, a replica
theory for the recurrent Heavy--Ball, Chebyshev, or PCG architectures developed
later.  The rigorous controller-dependent transfer audit is given in
\texttt{experiments/transformers/three\_controller\_encoder\_decoder\_generalization.tex}.
For Heavy--Ball and Chebyshev the Richardson spectral filter can be replaced
by a right-hand-side-independent polynomial.  For PCG this substitution is
invalid because the Krylov coefficients depend on the right-hand side.  The
gradient-flow ODEs in this manuscript also do not describe Muon updates.
\end{remark}
...
Once the resulting raw normalized spectral law $\nu$ is known, the
equivariant one-head preconditioner has an explicit closure.  With
$S/K\to\sigma$, limiting query law $\pi$, and bounded invariant score
$\ell(q,\lambda;\nu)$, define
\begin{align*}
    Z(q)&=\int e^{\ell(q,t;\nu)}\nu(dt),\\
    g(\lambda;\nu)
...
and depth.  No eigenvector or layer-indexed Krylov order parameter is added.
Gaussian forcing is used only for the explicit Wick covariance above; the
finite-dimensional spectral-risk predictor itself requires only an SPD
normal matrix and finite moments.
\end{remark}
...
the covariant ridge metric.  Let $(H_m,c_m)$ be the normal matrix and
right-hand side from an actual length-$m$ prompt.  Then
\[
    m^{-1}(H_m-\overline H_m(z))\longrightarrow0,
    \qquad
    m^{-1}(c_m-\overline c_m(z))\longrightarrow0
\]
almost surely.  If the population normal matrices remain uniformly SPD and
the invariant one-head score is Lipschitz on the resulting compact spectral
set, the effective spectral law, every fixed-depth HB or Chebyshev risk, and
the corresponding physical linearized risk computed from
$(\overline H_m,\overline c_m)$ converge to their prompt-law counterparts.
...
quadratic form $Y$ and its cross moment proves the two almost-sure limits.
Gaussian quadratic forms have finite moments of every order, so their
conditional central-limit fluctuations are $O_{\mathbb P}(m^{-1/2})$.
Uniform SPD, continuity of eigenvalues, and the Lipschitz spectral head pass
this convergence through the exact spectral pushforward and every
fixed-degree residual polynomial.  The physical statement follows by the
same argument for the bounded decoder Jacobian.  Standard argmin stability
