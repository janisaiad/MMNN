...
softmax-routed block before QR, \(A=H/s_H\), and \(r\) the fixed number of
...
\end{align}
If \(c_\star=\lambda_{\min}(C)\), the actual preconditioner is exactly
\begin{equation}
 B_\theta=s_H^{-1}\left[
...
requires the limits of \((K_r,J_r)\) and the cross measures of \(U\) with the
...
\[
 \operatorname{rank}(A_\theta-H/s_H)\leq2S,
 \qquad
 \sup_x|F_{A_\theta}(x)-F_{H/s_H}(x)|\leq 2S/K.
\]
