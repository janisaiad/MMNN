...
\begin{remark}[deformed PDE law]
For the GP-forced PDE prompt law, replace $\mu_{\rm MP,c}$ by the deformed
sample-covariance law induced by the population weak-row covariance $H(z)$,
and replace $\phi_t$ by the shrinkage map selected by the corresponding
replica saddle.  Equations \eqref{eq:rep-hb-distributional-risk}--
\eqref{eq:rep-hb-distributional-optimum} are unchanged.  No layer-indexed
order parameter is added because HB and Chebyshev remain fixed polynomials;
only the encoder/preconditioner spectral law changes.
\end{remark}
